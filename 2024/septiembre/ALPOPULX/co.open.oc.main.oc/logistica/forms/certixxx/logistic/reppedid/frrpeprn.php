<?php
      ...
      if (mysql_num_rows($xPedido) > 0) {
        while ($xRPE = mysql_fetch_array($xPedido)) {
          ...
          if ($xRPE["pedtipxx"] == "AUTOMATICA" && $xRPE["depnumxx"] != "") {
            ...
            if(in_array($xRPE['orvsap2x'], $mOrgVentas)){
              $cCodOrgVen = $mOrgVentas[$xRPE['orvsapxx']]['orvsapxx'];
            ...
            if(in_array($xRPE['ofvsap2x'], $mOfiVentas)){
              $cCodOfiVen = $mOfiVentas[$xRPE['ofvsapxx']]['ofvsapxx'];
            ...
            if(in_array($xRPE['closapxx'], $mCentLog)){
              $cCodCosLog = $mCentLog[$xRPE['closapxx']]['closapxx'];
            ...
            if(in_array($xRPE['secsapxx'], $mSector)){
              $cCodSect = $mSector[$xRPE['secsapxx']]['secsapxx'];
            ...
            $mData[$nInd_mData]['orgvntsx'] = $cCodOrgVen;  // ORGANIZACIÓN VENTAS
            $mData[$nInd_mData]['desorgvt'] = $cDesOrgVen;  // DESCRIPCIÓN ORG. VENTAS
            $mData[$nInd_mData]['oficvnts'] = $cCodOfiVen;  // OFICINA VTAS
            $mData[$nInd_mData]['desofcvt'] = $cDesOfiVen;  // DESCRIPCIÓN OFI. VENTAS
            $mData[$nInd_mData]['centlogx'] = $cCodCosLog;  // CENTRO LOGÍSTICO
            $mData[$nInd_mData]['desctlog'] = $cDesCosLog;  // DESCRIPCIÓN CENTRO LOGÍSTICO
            $mData[$nInd_mData]['sectorxx'] = $cCodSect;    // SECTOR
  ...
  if (count($mData) > 0) {
    ...
    $data .= '<table width="1024px" cellpadding="1" cellspacing="1" border="1" style="font-family:arial;font-size:12px;border-collapse: collapse;">';
      $data .= '<tr>';
        $data .= '<td colspan="42" style="font-size:14px;text-align:center;"><B>ALPOPULAR SA</B></td>';
      $data .= '</tr>';
      $data .= '<tr>';
        $data .= '<td colspan="42" style="font-size:14px;text-align:center;"><B>REPORTE PEDIDO</B></td>';
      $data .= '</tr>';
      $data .= '<tr>';
        $data .= '<td colspan="42" style="font-size:14px;text-align:left;"><B>Fecha Generaci&oacute;n: '.date('Y-m-d H:i:s').'</B></td>';
      $data .= '</tr>';
      ...
      for ($i=0; $i < count($mData); $i++) { 
        $data .= '<tr>';
          ...
          $data .= '<td width="200px" style="font-size:14px;text-align:left;mso-number-format:\'\@\'">'.$mData[$i]['clisapxx'].'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.$mData[$i]['clinomxx'].'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;mso-number-format:\'\@\'">'.$mData[$i]['orgvntsx'].'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.utf8_decode($mData[$i]['desorgvt']).'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;mso-number-format:\'\@\'">'.$mData[$i]['oficvnts'].'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.utf8_decode($mData[$i]['desofcvt']).'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;mso-number-format:\'\@\'">'.$mData[$i]['centlogx'].'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.utf8_decode($mData[$i]['desctlog']).'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;mso-number-format:\'\@\'">'.$mData[$i]['sectorxx'].'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.utf8_decode($mData[$i]['dessectx']).'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;mso-number-format:\'\@\'">'.$mData[$i]['condcomx'].'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;mso-number-format:\'\@\'">'.$mData[$i]['deposito'].'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;mso-number-format:\'\@\'">'.$mData[$i]['nummifxx'].'</td>';
          ...
          if ($numPedidoAnterior === $mData[$i]['numpedid']) {
            $j++;
          } else {
            $j = 1;
            $numPedidoAnterior = $mData[$i]['numpedid'];
          }

          $data .= '<td width="200px" style="font-size:14px;text-align:center;">'.$j.'</td>';
          ...
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.number_format($mData[$i]['pedbasex'], 2, '.', '').'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.number_format($mData[$i]['pedtarix'], 2, '.', '').'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.number_format($mData[$i]['pedcalcu'], 2, '.', '').'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.number_format($mData[$i]['pedminix'], 2, '.', '').'</td>';
          $data .= '<td width="200px" style="font-size:14px;text-align:left;">'.number_format($mData[$i]['pedvlrxx'], 2, '.', '').'</td>';
