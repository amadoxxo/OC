    ...
    function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
      ...
      if ($nSwicht == 0) {
        ...
        if ($mdb){
          ...
          $qInsert  = "INSERT INTO sys00004 (modidxxx, proidxxx, prodirxx, protipxx, proinixx, prodesxx, proordxx, proparxx, proverxx, proimgon, proimgof, proalcxx, proclixx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
          $qInsert .= "(\"1000\", \"3820\", \"condsaut\", \"2\", \"frcdsini.php\", \"Conceptos Contables Documentos Soporte\", \"3820\", \"\", \"SI\", \"btn_doc_cruce.gif\", \"btn_doc_cruce_disabled.gif\", \"\", \"\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
          ...
          $qInsert  = "INSERT INTO sys00005 (modidxxx, proidxxx, menidxxx, menformx, menopcxx, menordxx, mendesxx, menimgon, menimgof, menalcxx, mentipxx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
          $qInsert .= "(\"1000\", \"3820\", \"0\", \"\", \"CONSULTAR\", \"00000\", \"Consultar Concepto Contable\", \"\", \"\", \"\", \"\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $qInsert  = "INSERT INTO sys00005 (modidxxx, proidxxx, menidxxx, menformx, menopcxx, menordxx, mendesxx, menimgon, menimgof, menalcxx, mentipxx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
          $qInsert .= "(\"1000\", \"3820\", \"1\", \"frcdsnue.php\", \"NUEVO\", \"00001\", \"Nuevo Concepto Contable\", \"btn_doc_cruce.gif\", \"btn_doc_cruce_disabled.gif\", \"\", \"\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $qInsert  = "INSERT INTO sys00005 (modidxxx, proidxxx, menidxxx, menformx, menopcxx, menordxx, mendesxx, menimgon, menimgof, menalcxx, mentipxx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
          $qInsert .= "(\"1000\", \"3820\", \"2\", \"frcdsnue.php\", \"EDITAR\", \"00002\", \"Editar Concepto Contable\", \"\", \"\", \"\", \"\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $qInsert .= "(\"ADMIN\", \"1000\", \"3820\", \"0\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $qInsert  = "INSERT INTO sys00006 (usridxxx, modidxxx, proidxxx, menidxxx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
          $qInsert .= "(\"ADMIN\", \"1000\", \"3820\", \"1\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $qInsert  = "INSERT INTO sys00006 (usridxxx, modidxxx, proidxxx, menidxxx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
          $qInsert .= "(\"ADMIN\", \"1000\", \"3820\", \"2\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          while ($xRU = mysql_fetch_array($xUsuarios)) {
            $qInsert  = "INSERT INTO sys00006 (usridxxx, modidxxx, proidxxx, menidxxx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
            $qInsert .= "(\"{$xRU['USRIDXXX']}\", \"1000\", \"3820\", \"0\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
            $xInsert  = mysql_query($qInsert,_CONEXION_);
            if ($xInsert) {
              $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_);
              system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
