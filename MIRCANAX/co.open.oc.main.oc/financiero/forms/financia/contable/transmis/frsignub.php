...
<html>
	...
	<body>
		<?php
			...
			/**
			 * Buscando comprobantes marcados como reembolos de caja menor.
			 */
			$qComRCM  = "SELECT ";
			$qComRCM .= "CONCAT(comidxxx,\"-\",comcodxx) AS comidxxx ";
			$qComRCM .= "FROM $cAlfa.fpar0117 ";
			$qComRCM .= "WHERE ";
			$qComRCM .= "comidxxx = \"P\" AND ";
			$qComRCM .= "comtipxx = \"RCM\" ";
			$xComRCM = f_MySql("SELECT","",$qComRCM,$xConexion01,"");
			$mComRCM = array();
			while ($xRCR = mysql_fetch_array($xComRCM)) {
				$mComRCM[count($mComRCM)] = $xRCR['comidxxx'];
			}

			/**
			 * Buscando comprobantes marcados como Causacion Proveedor Empresa. 
			 */
			$qCauProEmp  = "SELECT ";
			$qCauProEmp .= "CONCAT(comidxxx,\"-\",comcodxx) AS comidxxx ";
			$qCauProEmp .= "FROM $cAlfa.fpar0117 ";
			$qCauProEmp .= "WHERE ";
			$qCauProEmp .= "comidxxx = \"P\" AND ";
			$qCauProEmp .= "comtipxx = \"CPE\" ";
			$xCauProEmp = f_MySql("SELECT","",$qCauProEmp,$xConexion01,"");
			$vCauProEmp = array();
			while ($xRDB = mysql_fetch_array($xCauProEmp)) {
				$vCauProEmp[count($vCauProEmp)] = $xRDB['comidxxx'];
			}
			...
			if ($nSwitch == 0) {
				...
				while ($xRCD = mysql_fetch_array($xCocDat)) {
					...
					$cNroCuota    = "";
					$dFecVenci    = "";
					$cTerId				= ($xRCD['comidxxx'] == "F") ? $xRCD['terid2cx'] : $xRCD['teridxxx'];
					...
					// Valida las cuentas por cobrar y por pagar para enviar el prefijo y consecutivo
					if ($mCuenta["{$xRCD['pucidxxx']}"]['pucdetxx'] == "C" || $mCuenta["{$xRCD['pucidxxx']}"]['pucdetxx'] == "P") {
						if ($xRCD['comidxxx'] == "F") {
							$cConsecutivo = $xRCD['comcscxx'];
							$cPrefijo     = $xRCD['resprexx'];
						} else {
							$cPrefijo 		= $xRCD['comidcxx'] . "-" . $xRCD['comcodcx'];
							$cConsecutivo = $xRCD['comcsccx'];
						}
					}

					$cCcSubcc = $xRCD['ccoidcab'] .'-'. ltrim($xRCD['sccidcab'], '0');
					// Para los comprobantes de reembolso de caja menor y causaciones proveedor empresa se debe enviar el centro y subcentro de costo del detalle
					if(in_array("{$xRCD['comidxxx']}-{$xRCD['comcodxx']}", $mComRCM) || in_array("{$xRCD['comidxxx']}-{$xRCD['comcodxx']}", $vCauProEmp)){
						$cCcSubcc = $xRCD['ccoidxxx'] .'-'. ltrim($xRCD['sccidxxx'], '0');
					}
          ...
          $mDatos[$nIndice][$nCountReg]['prefijox'] = $cPrefijo; //Prefijo
					...
					$mDatos[$nIndice][$nCountReg]['ccsubccx'] = $cCcSubcc; //Codigo centro/subcentro de costos
