<?php
  ...
  $mCliVen = explode("~",$vCocDat['CLIVENXX']);
  $vCliVen = $mCliVen[1];
                ...
                <td>
                  ...
                  <?php if($cTerId <> ""){ ?>
                      ...
                      <tr>
                        ...
                        <td class="name" width="25%"><br><?php echo $vCliVen ?></td>
      		            </tr>
                    </table>
    		          <?php } ?>
  		            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse: collapse; border-top:none; border-bottom: 1px solid black; border-left: 1px solid black; border-right: 1px solid black;">
  ...
  if ($nFilCta > 0 OR mysql_num_rows($xEstPro) > 0) {
	  ...
	  $header .= "\n";
		$data = '';
		$title = "ESTADO DE CUENTA POR $cTipoCta.xls";
		/**
        ...
        $data .= '<tr>';
         ...
         $data .= '<td colspan="'.($nColSpan-4).'">'.$vCliVen.'</td>';
        $data .= '</tr>';
	    }
    ...
    /* PINTA POR PDF */
		...
		if ($nFilCta > 0 OR mysql_num_rows($xEstPro) > 0) {
		  ...
		  class PDF extends FPDF {
            ...
            case "DETRLXXXXX":
            case "TETRLXXXXX":
              $this->Image($cRoot.$cPlesk_Skin_Directory.'/logobma.jpg',15,8,17,13);
            break;
						case "TEADIMPEXX": // ADIMPEX
						case "DEADIMPEXX": // ADIMPEX
						case "ADIMPEXX": // ADIMPEX
