  ...
  case "VALIDARDO":
    for ($i=0; $i<$_POST['nSecuencia']; $i++) {
      if ($_POST['cSucId' .($i+1)] != "" && $_POST['cDocId' .($i+1)] != "" && $_POST['cDocSuf' .($i+1)] != "") {
        ...
        $cTraSel .= "{$_POST['cSucId'.($i+1)]}~{$_POST['cDocId'.($i+1)]}~{$_POST['cDocSuf'.($i+1)]}|";
    ...
    if ($nSwitch == 0) {   ?>
      <script language="javascript">
        parent.fmwork.document.forms['frgrm']['cTramites'].value  = "<?php echo $cTraSel ?>";
        parent.fmwork.document.forms['frgrm']['nSecuencia'].value = "<?php echo $_POST['nSecuencia'] ?>";
