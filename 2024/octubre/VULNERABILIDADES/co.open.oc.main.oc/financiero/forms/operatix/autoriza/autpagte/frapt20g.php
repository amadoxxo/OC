<?php
  namespace openComex;
  /**
   * Permite cargar la grilla de los DO.
          ...
          if ($vTramPeg[$nT] != "") {
            ...
            while ($xRT = mysql_fetch_array($xTramites)) {
              ...
              <script languaje = "javascript">
                ...
                if(parent.window.opener.fnValidateExist("<?php echo $xRT['sucidxxx'] ?>", "<?php echo $xRT['docidxxx'] ?>", "<?php echo $xRT['docsufxx'] ?>")) {
                  alert('El DO <?php echo $xRT['docidxxx'] ?> no se agrego, ya existe en la grilla.');
                } else {
                  var nSecuencia = parent.window.opener.document.forms['frgrm']['nSecuencia'].value;
                  if(parent.window.opener.document.forms['frgrm']['cSucId' +nSecuencia].value != '') {
