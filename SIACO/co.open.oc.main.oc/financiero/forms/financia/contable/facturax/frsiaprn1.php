<?php
  /**
	 * Imprime Factura de Venta Siaco.
  	...
  	if ($mPrints[$i][0]!=""){
		  ...
		  $qValCsc  = "SELECT comidxxx,comcodxx,comcscxx,comcsc2x ";
			...
			if (mysql_num_rows($xValCsc) > 1) {
				$nSwitch = 1;
				$cMsj .= "El Documento [{$mPrints[$i][0]}-{$mPrints[$i][1]}-{$mPrints[$i][2]}] No se puede Imprimir porque su Consecutivo se Encuentra Repetido en el Modulo de Contabilidad.\n";
			}
  	}
    ...
    if ($vMemo[$u]!=""){
      ...
      ## Select a la 1001 para traer el campo que se marca cuando se ha impreso factura##
    	...
    	if ($nFilCoc > 0) {
    	  ...
    	  if($vCocDat['comprnxx']=="IMPRESO" && $vCocDat['regestxx'] != "INACTIVO"){
  ...
  if($permisos==1) {
    $nSwitch=1;
    $cMsj .= "Las Siguientes Facturas No tienen Permiso de Impresion [$zCadPer].\n";
  }
 ...
 if($nSwitch == 0){
		class PDF extends FPDF {
 			function Header() {
 				...
 				global $cDocId;  global $vCiuDat; global $vDceDat; global $vCccDat; global $cFormaPag; global $vMedPag;
 				global $vConDat; global $vCcoDat; global $cDeposito; global $mIngTer; global $cPaiOri; global $cPaiOriDes;
 				global $gCorreo; global $cNomDirCue; global $cSucId; global $cPedido; global $cPedOrd; global $cLugIngDes;
        global $OPENINIT; global $gOrigen; global $vSysStr;
        ...
        if ($_COOKIE['kModo'] == "VERFACTURA" || $vCocDat['regestxx'] == "PROVISIONAL") {
        ...
        if ($vResDat['resvigme'] != "") {
      	...
      	$this->RoundedRect($posx+90, $posy, 50, 8, 3, '1', 'F');
				...
				} elseif ($vCocDat['terid2xx'] == '900736182') {
          ...
          $this->Cell(30,4, $vSysStr['siacosia_facturacion_referencia_apple'],0,0,'L');
 			...
 			function Footer() {
 				global $gCorreo; global $cNomCopia; global $vCocDat; global $cPlesk_Skin_Directory; global $vSysStr;
      ...
      $nGenerar = 0;
        ...
        $mPccTP  = array(); //Tasa Pactada, cuando la moneda es en COP y el pago a tercero tiene tasa de cambio pactada
	  		...
	  		global $xConexion01; global $cAlfa; global $vObsComp; global $xObsComp;global $vUserNom; global $mResFac;

	  		//Tener en cuenta que para el caso del envio del correo solo se genera la original
	  		$nCopias = ($gCorreo == 0) ? (($vCccDat['ccccopfa'] == "") ? 3 : ($vCccDat['ccccopfa']+2)) : 1;
          ...
          $nVrlTotalBaseIva	= 0;
	  			...
	  			while ($nBanPCC == 0 || $nBanIP == 0) {
	  				...
	  				switch($y) {
	  					case $nConta:
	  						$cNomCopia = ($gCorreo == 0) ? "CARTERA" : "";
	  					break;
	  					case $nCarte:
	  						$cNomCopia = ($gCorreo == 0) ? "CONTABILIDAD" : "";
	  					break;
	  					default:
	  						$cNomCopia = ($gCorreo == 0) ? "CLIENTE" : "";
	  					break;
	  				}
	  				##Codigo Para impresion de Copias de Factura ##
		...
		$cFile = f_Buscar_Niveles_Hasta_Opencomex(getcwd()).$vSysStr['system_download_directory']."/pdf_".$_COOKIE['kUsrId']."_".str_pad($nCanFac,3,"0",STR_PAD_LEFT)."_".date("YmdHis").".pdf";
	  $pdf->Output($cFile);
