...
for ($i=0;$i<count($aconn);$i++){
	$vdb  = $aconn[$i]['DB'];
	$cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\Agrega un campo a la tabla fpar0151 en: ".$vdb;
	...
	function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
		...
		if ($nSwitch == 0) {
			...
			if ($mdb){
				...
				$qAlter  = "ALTER TABLE fpar0151 ADD cccvlcto TEXT NOT NULL COMMENT \"Valores Unitarios Conceptos Pagos a Terceros\" AFTER cccagrta";
				$xAlter  = mysql_query($qAlter,_CONEXION_);

				if ($xAlter) {
					$cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Creo el Campo fpar0151.cccvlcto en: ".$pdb;
					system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
				} else {
					$nSwitch = 1;
					$cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError Al Crear el Campo fpar0151.cccvlcto en: ".$pdb;
