	...
	<html>
		<head>
	   	...
	   	function f_Guardar(){
		   	...
		   	if (cadeni.length > 1)	{
		   		var cRuta = "frtercav.php?cTerId=<?php echo $cTerId ?>&cCadena="+cadeni+"&cCliCon="+window.opener.document.forms['frgrm']['cCliCon'].value+"&tipsave=5";
	        ...
	        if(http_request.readyState==1){          
	          ...
	          if (http_request.status == 200) {
	            if(http_request.responseText!=""){
	              ...
	              if (mRetorno[0] == "true") {
	                window.opener.document.forms['frgrm']['cCliCon'].value = mRetorno[1];
	                window.opener.f_CargarContactos();                
	                window.close();
	              } else {
	                alert(mRetorno[1]);
	              }
	  ...
	  <body topmargin = 0 leftmargin = 0 margnwidth = 0 marginheight = 0 style = 'margin-right:0'>
	  	<?php
      $mCadena = explode("~",$gCliCon);
      $mContactos = array();
      for($i=0;$i<count($mCadena);$i++){
       if ($mCadena[$i] != "") {
         $mContactos[count($mContactos)] = $mCadena[$i];
       }
      }     
      ?>
					...
					<td>
						<fieldset>
	  					...
	  					<form name = "frgrm" action = "" method = "post" target = "fmpro">
	  						<?php
	  						$qCliDat  = "SELECT CLIIDXXX,CLINOMXX,CONCAT(CLIAPE1X,' ',CLIAPE2X,' ',CLINOM1X,' ',CLINOM2X) AS NOMBRE,REGESTXX ";
								$qCliDat .= "FROM $cAlfa.SIAI0150 ";
								$qCliDat .= "WHERE CLITCONX = 'SI' AND ";
								$qCliDat .= "REGESTXX = \"ACTIVO\" ORDER BY NOMBRE";
								...
								if (mysql_num_rows($xCliDat) > 0) {
					    			...
					    			<table cellspacing = "0" cellpadding = "1" border = "1" width = "450">
							          ...
							          $vb   = $serv;
											  $cvb  = 0;
                        if (in_array($vb,$mContactos) == true) {
                         $cvb = 1;
                        }							          
							          if($zRCom['CLINOMXX'] == ''){
							           $zRCom['NOMBREXX'] = $zRCom['NOMBRE'];
							          }else{
							             $zRCom['NOMBREXX'] = $zRCom['CLINOMXX'];
							           }
							          if ($cvb == 0)	{
							          	$y ++;
							          	$cont++;
								          ...
								          if($y % 2 == 0) {
								          	$zColor = "{$vSysStr['system_row_par_color_ini']}";
											   	...
											   	<tr bgcolor = "<?php echo $zColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $zColor ?>')">
														...
														<td style='width: 280px' Class = 'letra7'><?php echo substr($zRCom['NOMBREXX'],0,45) ?></td>
									...
									if ($cont == 0)	{
										?>
										<script languaje='javascript'>
											alert('Ya tiene asignados todos los Contactos Existentes.');
											window.close();
										</script>
										<?php
									}
