<?php
  namespace openComex;
   /**
	 * Liberar Formularios.
	 * --- Descripcion: me Lista los formularios legalizados y  no legalizados que hayan sido asigandos a ese Director
	 * de cuenta. y me da la opcion de chekear los que quiera Liberar.
  	    ...
  	    $cDirector="";
       	$qDatUsr  = "SELECT * ";
		    ...
		    $qFoiSer .= "FROM $cAlfa.ffoi0000 ";
				$qFoiSer .= "WHERE diridxxx = \"$cDirector\" AND regestxx = \"CONDO\" AND doccomex <> \"\"";
 			  if($_POST['cTipPro']!=""){
 			    $qFoiSer .= "AND ptoidxxx=\"{$_POST['cTipPro']}\" ";
 			  }
				$qFoiSer .= "ORDER BY CONVERT(seridxxx,signed) ASC ";
				//wMenssage(__FILE__,__LINE__,$qFoiSer);
				$xFoiSer  = f_MySql("SELECT","",$qFoiSer,$xConexion01,"");
				//wMenssage(__FILE__,__LINE__,mysql_num_rows($xFoiSer));
				/* Cargo la Matriz con los ROWS del Cursor */
				$i=0;
				while ($xRFS = mysql_fetch_array($xFoiSer)) {
					$mMatrizTmp[$i] = $xRFS;
					$i++;
				}
      ...
      <script languaje="javascript">
       	...
       	<table width="60%" cellspacing="0" cellpadding="0" border="0">
         	<tr>
	       	  <td>
				      <fieldset>
   			        <legend>Formularios Asignados al Usuario con DO: <font color=red><b><?php echo $cNomUsr; ?></legend>
     	       		<center>
														...
														switch ($xRBA['menopcxx']) {
															...
															<img src = "<?php echo $cPlesk_Skin_Directory ?>/page_go.png"
															  onClick = "javascript:f_Verificar_Check();
															                        ...
															                        document.forms['frgrm'].method='post';
															                        document.forms['frgrm'].target='fmpro';
															                        document.forms['frgrm'].submit();"
															                        style = "cursor:hand" title="Liberar Formulario" id="IdImg">
															-->
                              <input class="name" name="IdImg" type="button" value="Liberar" style="cursor:pointer"
															  onClick = "javascript:f_Verificar_Check();
															                        f_Carga_Variable(document.forms['frgrm']['nRecords'].value);
															                        document.forms['frgrm'].action='frforagr.php';
															                        document.forms['frgrm'].target='fmpro';
         	         ...
         	         <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
             	      ...
             	      <tr bgcolor = 'white'>
 	                    	...
 	                    	if ($i < count($mMatrizTra)) { // Para Controlar el Error
	                   	    ...
	                   	    if($y % 2 == 0) {
		                      	...
		                      	<td class="letra7" width="20%"><?php echo $mMatrizTra[$i]['ptoidxxx'];?></td>
	       	                	<td class="letra7" width="30%"><?php echo $mMatrizTra[$i]['ptodesxx'] ?></td>
	       	                	<td class="letra7" width="20%"><?php echo $mMatrizTra[$i]['seridxxx'] ?></td>
	       	                	<td class="letra7" width="20%"><?php echo $mMatrizTra[$i]['doccomex'] ?></td>
	        	              	<td class="letra7" width="10%" align="right">
	        	              	  <input type="checkbox" name="cCheck"  value = "<?php echo $mMatrizTra[$i]['seridxxx'].'~'.$mMatrizTra[$i]['ptoidxxx'].'~'.$mMatrizTra[$i]['doccomex'] ?>"
 	                    ...
 	                    <?php switch ($_COOKIE['kModo']) {
			                  ...
			                  case "LIBERAR":?>
				                  <script languaje = "javascript">
					                  document.forms['frgrm']['gTipSav'].value    = "LIBERAR";
				                  </script>
			                    <?php
			                  break;
                      } ?>
