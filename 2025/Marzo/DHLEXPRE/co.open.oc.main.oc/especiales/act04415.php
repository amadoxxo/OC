  <?php
    ...
    define("_FILELOG_","act04415.log"); // Archivo de LOG
    ...
    for ($i=0;$i<count($aconn);$i++){
      $vdb  = $aconn[$i]['DB'];
      $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\CREA OPCION DE MENU - REPORTE DE CONDICIONES COMERCIALES DHL EXPRESS ".$vdb;
      ...
      function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
        ...
        if ($nSwicht == 0) {
          ...
          if ($mdb){
            ...
            if ($pdb == "DEDHLEXPRE" || $pdb == "TEDHLEXPRE" || $pdb == "DHLEXPRE") {
              ...
              $qInsert .= ") VALUES (";
              $qInsert .= "\"3000\", \"150\", \"25\", \"frrcdnue.php\", \"REPORTECONCOMDHL\", \"00025\", \"Reporte Condiciones Comerciales DHL Express\", \"btn_siteapp-pool_bg.gif\", \"btn_siteapp-pool-disabled_bg.gif\", \"\", \"\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
              $xInsert  = mysql_query($qInsert,_CONEXION_);
              if ($xInsert) {
                $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_)."\tSe creo la Opcion de Menu Reporte de Condiciones Comerciales DHL Express";
                system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
              } else {
                $nSwicht = 1;
                $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError Al Crear la Opcion Reporte de Condiciones Comerciales DHL Express";
                system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
              }
              ...
              $qInsert .= ") VALUES (";
              $qInsert .= "\"ADMIN\", \"3000\", \"150\", \"25\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
              $xInsert  = mysql_query($qInsert,_CONEXION_);
              if ($xInsert) {
              ...
              while ($xRU = mysql_fetch_array($xUsuarios)) {
                ...
                $qInsert .= "(\"{$xRU['USRIDXXX']}\", \"3000\", \"150\", \"25\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
                $xInsert  = mysql_query($qInsert,_CONEXION_);
                if ($xInsert) {
                  $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_affected_rows(_CONEXION_);
                  system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
