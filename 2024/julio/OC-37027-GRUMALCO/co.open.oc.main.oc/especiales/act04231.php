...
define("_FILELOG_","act04193.log"); // Archivo de LOG
...
for ($i=0;$i<count($aconn);$i++){
  $vdb  = $aconn[$i]['DB'];
  $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\Agrega un campo a la tabla sys00005 en: ".$vdb;
  ...
  function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
    ...
    if ($nSwitch == 0) {
      ...
      if ($mdb){
        ...
        $qInsert  = "INSERT INTO $pdb.sys00005 (modidxxx, proidxxx, menidxxx, menformx, menopcxx, menordxx, mendesxx, menimgon, menimgof, menalcxx, mentipxx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
        $qInsert .= "(\"1000\", \"90\", \"170\", \"\", \"BORRARDISCONFORMIDAD\", \"00170\", \"Borrar Disconformidad\", \"\", \"\", \"\", \"\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
        $xInsert = mysql_query($qInsert,_CONEXION_);
        if ($xInsert) {
          $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Inserto el Campo en la tabla sys00005 para Borrar Disconformidad en: ".$pdb;
          system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
        } else {
          $nSwitch = 1;
          $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError Al Insertar en la tabla sys00005 para Borrar Disconformidad en: ".$pdb;
          system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
        }

        $qInsert  = "INSERT INTO $pdb.sys00006 (usridxxx, modidxxx, proidxxx, menidxxx, regfcrex, reghcrex, regfmodx, reghmodx, regestxx) VALUES ";
        $qInsert .= "(\"ADMIN\", \"1000\", \"90\", \"170\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
        $xInsert = mysql_query($qInsert,_CONEXION_);
        if ($xInsert) {
          $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Inserto el Campo en la tabla sys00006 para Borrar Disconformidad en: ".$pdb;
          system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
        } else {
          $nSwitch = 1;
          $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError Al Insertar en la tabla sys00006 para Borrar Disconformidad en: ".$pdb;
          system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
        }
        ...
        while ($xRU = mysql_fetch_array($xUsuarios)) {
          ...
          $qInsert .= "(\"{$xRU['USRIDXXX']}\", \"1000\", \"90\", \"170\", NOW(), NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
