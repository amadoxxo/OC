<?php
/**
	 * Tracking Autorizacion Excluir conceptos de cobro.
	 * --- Descripcion: Parametrica para autorizar la exclusion de conceptos de Cobro.
...
<html>
  	...
  	<LINK rel = 'stylesheet' href = '<?php echo $cSystem_Libs_JS_Directory ?>/estilo.css'>
   	...
   	<script language="javascript">
    	...
    	function f_Ver(xSucId,xDocId,xDocSuf) {
      	var ruta = "fraecnue.php?gSucId="+xSucId+'&gDocId='+xDocId+'&gDocSuf='+xDocSuf;
      	document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
      	document.cookie="kMenDes=Ver Autorizacion Excluir Concepto de Cobro;path="+"/";
				...
				switch (document.forms['frgrm']['vRecords'].value) {
					case "1":
						if (document.forms['frgrm']['oCheck'].checked == true) {
							...
							var ruta = "fraecnue.php?gSucId="+zMatriz[0]+"&gDocId="+zMatriz[1]+"&gDocSuf="+zMatriz[2];
      	      document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
      	      document.cookie="kMenDes=Editar Autorizacion Excluir Concepto de Cobro;path="+"/";
	    ...
	    function f_Anular(xModo) {
				switch (document.forms['frgrm']['vRecords'].value) {
					case "1":
						if (document.forms['frgrm']['oCheck'].checked == true) {
							var zMatriz = document.forms['frgrm']['oCheck'].id.split('~');
							if (confirm("Esta Seguro de Anular la Autorizacion Excluir Conceptos de Cobro para el Do "+zMatriz[0]+"-"+zMatriz[1]+"-"+zMatriz[2]+" ")) {
							  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
							  document.cookie="kModo="+xModo+";path="+"/";
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
		<form name = "frestado" action = "fraecgra.php" method = "post" target="fmpro">
		...
		<form name = "frgrm" action="fraecini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $qDatDce .= "$cAlfa.sys00121.docfmaxx, "; // Tipo de Facturacion manual para un tramite.
        ...
        if (substr_count($_POST['cOrderByOrder'],"clinomxx") > 0) {
          $qDatDce .= "LEFT JOIN $cAlfa.SIAI0150 ON $cAlfa.sys00121.cliidxxx = $cAlfa.SIAI0150.CLIIDXXX ";
				...
				$qDatDce .= "WHERE ";
				$qDatDce .= "$cAlfa.sys00121.doctexxx != \"\" ";
        if ($_POST['vSearch'] != "") {
					$qDatDce .= "AND (";
          ...
          $qDatDce .= "$cAlfa.sys00121.docfmaxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qDatDce .= "$cAlfa.sys00121.cliidxxx LIKE \"%{$_POST['vSearch']}%\" OR ";

          if ($cCliIdSearch != "") {
            $qDatDce .= "$cAlfa.sys00121.cliidxxx IN ($cCliIdSearch) OR ";
          }
          $qDatDce .= "$cAlfa.sys00121.regestxx LIKE \"%{$_POST['vSearch']}%\") ";
				}
        ...
        $qDatDce .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
				$xDatDce  = f_MySql("SELECT","",$qDatDce,$xConexion01,"");
