...
/**
 * Tracking Sector.
 * --- Este programa permite realizar consultas rapidas Sector que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xSecSap) {
        var cPathUrl = "frsecnue.php?cSecSap="+xSecSap;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Sector;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frsecnue.php?cSecSap="+mMatriz[0];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Sector;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de el Sector ["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
                  document.forms['frestado']['cSecSap'].value=mMatriz[0];
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frsecgra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cSecSap" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frsecini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mSector = array();
        $qSector  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qSector .= "$cAlfa.lpar0009.secsapxx, ";
        $qSector .= "$cAlfa.lpar0009.secdesxx, ";
        $qSector .= "$cAlfa.lpar0009.regusrxx, "; // Código Usuario
        $qSector .= "$cAlfa.lpar0009.regfcrex, "; // Fecha de creación
        $qSector .= "$cAlfa.lpar0009.reghcrex, "; // Hora de creación
        $qSector .= "$cAlfa.lpar0009.regfmodx, "; // Fecha de modificación
        $qSector .= "$cAlfa.lpar0009.reghmodx, "; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qSector .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qSector .= "$cAlfa.lpar0009.regestxx ";
        $qSector .= "FROM $cAlfa.lpar0009 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qSector .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0009.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qSector .= "WHERE ";
          $qSector .= "$cAlfa.lpar0009.secsapxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qSector .= "$cAlfa.lpar0009.secdesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qSector .= "$cAlfa.lpar0009.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qSector .= "$cAlfa.lpar0009.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qSector .= "$cAlfa.lpar0009.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qSector .= "$cAlfa.lpar0009.regusrxx IN ($cNombreSearch) OR ";
          }
          $qSector .= "$cAlfa.lpar0009.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        $qSector .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xSector = mysql_query($qSector, $xConexion01, true, $cIdCountRow);
        //echo $qSector." ~ ".mysql_num_rows($xSector);
        ...
        while ($xRDC = mysql_fetch_array($xSector)) {
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>Sector (<?php echo $nRNR?>)</legend>
                <center>
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="13%">
                        <a href = "javascript:fnOrder_By('onclick','secsapxx');" title="Ordenar">Cod Sap</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "secsapxx">
                        <input type = "hidden" name = "secsapxx" value = "<?php echo $_POST['secsapxx'] ?>" id = "secsapxx">
                        <script language="javascript">fnOrder_By('','secsapxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mSector) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mSector);$i++) {
                        if ($i < count($mSector)) { // Para Controlar el Error
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mSector[$i]['secsapxx']?>')>
                                                        <?php echo $mSector[$i]['secsapxx']?>
                            </a>
                          <td class="letra7" width="40%"><?php echo $mSector[$i]['secdesxx'] ?></td>
                          <td class="letra7" width="18%"><?php echo substr($mSector[$i]['usrnomxx'],0,20) ?></td>
                          <td class="letra7" width="07%"><?php echo $mSector[$i]['regfcrex'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mSector[$i]['reghcrex'] ?></td>
                          <td class="letra7" width="07%"><?php echo $mSector[$i]['regfmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mSector[$i]['reghmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mSector[$i]['regestxx'] ?></td>
                          <td Class="letra7" width="02%" align="right">
                            <input type="checkbox" name="oCheck" value = "<?php echo count($mSector) ?>"
                            id="<?php echo $mSector[$i]['secsapxx'].'~'.
                                            $mSector[$i]['regestxx'] ?>"
                            onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mSector) ?>'">
                          </td>
                        </tr>
                        <?php $y++;
                        }
