<?php
    ...
    function Header(){
      ...
      if ($cTarTip == "CLIENTE") {
        ...
        if($vSysStr['system_control_vigencia_tarifas'] != "SI"){
          ...
          $this->dTarFevDe = $vConDat['cccfdcxx'];
          $this->dTarFevHa = $vConDat['cccfhcxx'];
        }
      ...
      if ($cAlfa == "TRLXXXXX" || $cAlfa == "TETRLXXXXX" || $cAlfa == "DETRLXXXXX") {
        ...
        if ($cTarTip == "CLIENTE") {
          $this->setY(35);
          $this->MultiCell(190,5,"VIGENCIA DESDE : ".$this->dTarFevDe." HASTA: ".$this->dTarFevHa,'B','C');
          $this->SetFont('arial','',7);
          $this->setY(42);
          $this->Cell(100,3,'CLIENTE: '.$zRCli['CLINOMXX']);
          $this->Cell(50,3,'NIT: '.$zRCli['CLIIDXXX']."-".f_Digito_Verificacion($zRCli['CLIIDXXX']));
  ...
  // Si aplican vigencias deben imprimirse las tarifas según la vigencia
  $mVigencias = array();
  if($vSysStr['system_control_vigencia_tarifas']=="SI"){
    $qTarifas  = "SELECT tarfevde,tarfevha ";
    $qTarifas .= "FROM ";
    $qTarifas .= "$cAlfa.fpar0131 ";
    $qTarifas .= "WHERE ";
    $qTarifas .= "cliidxxx = \"$cCliId\" AND ";
    $qTarifas .= "tartipxx = \"$cTarTip\" ";
    if($gSerTop != "TODOS"){
      $qTarifas .= "AND fcotopxx = \"$gSerTop\" ";
    }
    if ($dTarFevDe != "" && $dTarFevDe != "0000-00-00" && $dTarFevHa != "" && $dTarFevHa != "0000-00-00") {
      $qTarifas .= "AND tarfevde >= \"$dTarFevDe\" ";
      $qTarifas .= "AND tarfevha <= \"$dTarFevHa\" ";
    }
    if($gOrdBy != "TODOS"){
      $qTarifas .= "AND regestxx = \"$gOrdBy\" ";
    }
    $qTarifas .= "GROUP BY tarfevde,tarfevha ";
    $qTarifas .= "ORDER BY tarfevde";
    $xTarifas  = f_MySql("SELECT","",$qTarifas,$xConexion01,"");
    // echo $qTarifas."~".mysql_num_rows($xTarifas);
    while ($xRT = mysql_fetch_array($xTarifas)){
      $nInd_mVigencias = count($mVigencias);
      $mVigencias[$nInd_mVigencias][0] = $xRT['tarfevde'];
      $mVigencias[$nInd_mVigencias][1] = $xRT['tarfevha'];
    }
  } else {
    $nInd_mVigencias = count($mVigencias);
    $mVigencias[$nInd_mVigencias][0] = "0000-00-00";
    $mVigencias[$nInd_mVigencias][1] = "0000-00-00";
  }

  for ($nT = 0; $nT < count($mVigencias); $nT++){
    // Asignado las fechas de vigencia para el encabezado
    $pdf->dTarFevDe = $mVigencias[$nT][0];
    $pdf->dTarFevHa = $mVigencias[$nT][1];
    ...
    $qSqlCab .= "WHERE ";
    $qSqlCab .= "$cAlfa.fpar0131.cliidxxx = \"$cCliId\" AND ";
    $qSqlCab .= "$cAlfa.fpar0131.tartipxx = \"$cTarTip\" ";
    // Verifico si marcaron TODOS en tipo de OPERACION
    if($gSerTop != "TODOS"){
      $qSqlCab .= "AND $cAlfa.fpar0131.fcotopxx = \"$gSerTop\" ";
    }
    if($vSysStr['system_control_vigencia_tarifas']=="SI"){
      $qSqlCab .= "AND $cAlfa.fpar0131.tarfevde = \"{$mVigencias[$nT][0]}\" ";
      $qSqlCab .= "AND $cAlfa.fpar0131.tarfevha = \"{$mVigencias[$nT][1]}\" ";
    }
    // Verifico si marcaron TODOS en ESTADO
    if($gOrdBy != "TODOS"){
      $qSqlCab .= " AND $cAlfa.fpar0131.regestxx = \"$gOrdBy\" ";
    }
    $qSqlCab .= "ORDER BY $cAlfa.fpar0131.fcotopxx,$cAlfa.fpar0131.fcotptxx,ABS($cAlfa.fpar0131.fcotpixx) ASC ";
    $xSqlCab  = f_MySql("SELECT","",$qSqlCab,$xConexion01,"");
    // echo $qSqlCab."~".mysql_num_rows($xSqlCab)."<br><br>";
    ...
    // Fin Seleccion de Tablas para ALPOPULX y DEMAS
