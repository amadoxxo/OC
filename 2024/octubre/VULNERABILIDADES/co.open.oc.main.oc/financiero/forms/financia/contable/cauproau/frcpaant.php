  			...
  			while ($xRPA = mysql_fetch_array($xPerAnt)) {
  				if ($xRPA['peranoxx'].$xRPA['permesxx'] < $cPerAno.$cPerMes) { ?>
	  				<script languaje = "javascript">
	  				  parent.fmwork.document.frgrm.dComFec.options[<?php echo $i ?>] = new Option('<?php echo $xRPA['peranoxx'].'-'.$xRPA['permesxx'].'-'.date( "d", mktime(0, 0, 0, $xRPA['permesxx'] + 1, 0, $xRPA['peranoxx']))?>','<?php echo $xRPA['peranoxx'].'-'.$xRPA['permesxx'].'-'.date( "d", mktime(0, 0, 0, $xRPA['permesxx'] + 1, 0, $xRPA['peranoxx']))?>');
	  				</script>
	  				<?php $i++;
  				}
  			}
