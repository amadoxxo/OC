<?php
	/**
	 * Tracking Descuentos
	 * --- Descripcion: Parametrica para Creacion/Edicion Descuento
...
<html>
	<head>
		...
		<script language="javascript">
			...
			function fnVer(xDesId) {
				var ruta = "frdesnue.php?gDesId="+xDesId;
				document.cookie="kIniAnt=dhlexpre/descuent/<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
				...
				switch (document.forms['frnav']['vRecords'].value) {
					case "1":
						if (document.forms['frnav']['oCheck'].checked == true) {
							...
							var ruta = "frdesnue.php?gDesId="+zMatriz[0];
							document.cookie="kIniAnt=dhlexpre/descuent/<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
	...
	<body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
		<form name = "frestado" action = "frdesgra.php" method = "post" target="fmpro">
			<input type = "hidden" name = "cDesId"  value = "">
			<input type = "hidden" name = "cRegEst" value = "">
		</form>

		<form name = "frnav" action="frdesini.php" method="post" target="fmwork">
			...
			<?php
				...
				if ($_POST['vSearch'] != "") {
					$qDescuento .= "WHERE ";
					$qDescuento .= "($cAlfa.fpar0164.desidxxx LIKE \"%{$_POST['vSearch']}%\" OR ";
					$qDescuento .= "$cAlfa.fpar0164.descodxx LIKE \"%{$_POST['vSearch']}%\" OR ";
					$qDescuento .= "$cAlfa.fpar0164.seridxxx LIKE \"%{$_POST['vSearch']}%\" OR ";
					$qDescuento .= "$cAlfa.fpar0129.serdespx LIKE \"%{$_POST['vSearch']}%\" OR ";
					$qDescuento .= "$cAlfa.fpar0129.serdesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
					$qDescuento .= "$cAlfa.fpar0164.fcoidxxx LIKE \"%{$_POST['vSearch']}%\" OR ";
					$qDescuento .= "$cAlfa.fpar0130.fcodesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
					$qDescuento .= "$cAlfa.fpar0164.desporce LIKE \"%{$_POST['vSearch']}%\" OR ";
					$qDescuento .= "$cAlfa.fpar0164.regestxx LIKE \"%{$_POST['vSearch']}%\") ";
				}
					...
					if ($vOrderByOrder[$z] != "") {
						if ($_POST[$vOrderByOrder[$z]] != "") {
							if (substr_count($_POST[$vOrderByOrder[$z]], "regfcrex") > 0) {
								...
								$cOrdFecCre = str_replace("regfcrex", "CONCAT(fpar0164.regfcrex,\" \",fpar0164.reghcrex)", $_POST[$vOrderByOrder[$z]]);
								$cOrderBy .= $cOrdFecCre;
							} else if (substr_count($_POST[$vOrderByOrder[$z]], "regfmodx") > 0) {
								//Ordena por "regfmodx regfmodx"
								$cOrdFecMod = str_replace("regfmodx", "CONCAT(fpar0164.regfmodx,\" \",fpar0164.reghmodx)", $_POST[$vOrderByOrder[$z]]);
								$cOrderBy .= $cOrdFecMod;
				...
				if (strlen($cOrderBy)>0) {
					...
					$cOrderBy = "ORDER BY fpar0164.regfmodx DESC, fpar0164.reghmodx DESC";
				}
									...
									<table cellspacing="0" width="100%">
										<tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
											<td class="name" width="5%">
												<a href = "javascript:fnOrderBy('onclick','desidxxx');" title="Ordenar">Id</a>&nbsp;
												<img src="<?php echo $cPlesk_Skin_Directory_New ?>/spacer.png" border="0" width="11" height="9" title = "" id = "desidxxx">
												<input type = "hidden" name = "desidxxx" value = "<?php echo $_POST['desidxxx'] ?>" id = "desidxxx">
												<script language="javascript">fnOrderBy('','desidxxx')</script>
											</td>
											<td class="name" width="8%">
												<a href = "javascript:fnOrderBy('onclick','descodxx');" title="Ordenar">C&oacute;digo</a>&nbsp;
												<img src="<?php echo $cPlesk_Skin_Directory_New ?>/spacer.png" border="0" width="11" height="9" title = "" id = "descodxx">
												<input type = "hidden" name = "descodxx" value = "<?php echo $_POST['descodxx'] ?>" id = "descodxx">
												<script language="javascript">fnOrderBy('','descodxx')</script>
											</td>
											<td class="name" width="6%">
												<a href = "javascript:fnOrderBy('onclick','seridxxx');" title="Ordenar">Id</a>&nbsp;
												<img src="<?php echo $cPlesk_Skin_Directory_New ?>/spacer.png" border="0" width="11" height="9" title = "" id = "seridxxx">
												<input type = "hidden" name = "seridxxx" value = "<?php echo $_POST['seridxxx'] ?>" id = "seridxxx">
												<script language="javascript">fnOrderBy('','seridxxx')</script>
											</td>
											<td class="name" width="20%">
												<a href = "javascript:fnOrderBy('onclick','serdesxx');" title="Ordenar">Servicio</a>&nbsp;
												<img src="<?php echo $cPlesk_Skin_Directory_New ?>/spacer.png" border="0" width="11" height="9" title = "" id = "serdesxx">
											...
											<td class="name" width="6%">
												<a href = "javascript:fnOrderBy('onclick','fcoidxxx');" title="Ordenar">Id</a>&nbsp;
												<img src="<?php echo $cPlesk_Skin_Directory_New ?>/spacer.png" border="0" width="11" height="9" title = "" id = "fcoidxxx">
												<input type = "hidden" name = "fcoidxxx" value = "<?php echo $_POST['fcoidxxx'] ?>" id = "fcoidxxx">
												<script language="javascript">fnOrderBy('','fcoidxxx')</script>
											</td>
											<td class="name" width="20%">
												<a href = "javascript:fnOrderBy('onclick','fcodesxx');" title="Ordenar">Forma Cobro</a>&nbsp;
												<img src="<?php echo $cPlesk_Skin_Directory_New ?>/spacer.png" border="0" width="11" height="9" title = "" id = "fcodesxx">
												<input type = "hidden" name = "fcodesxx" value = "<?php echo $_POST['fcodesxx'] ?>" id = "fcodesxx">
												<script language="javascript">fnOrderBy('','fcodesxx')</script>
											</td>
											<td class="name" width="8%">
												<a href = "javascript:fnOrderBy('onclick','desporce');" title="Ordenar">Descuento %</a>&nbsp;
												<img src="<?php echo $cPlesk_Skin_Directory_New ?>/spacer.png" border="0" width="11" height="9" title = "" id = "desporce">
												<input type = "hidden" name = "desporce" value = "<?php echo $_POST['desporce'] ?> %" id = "desporce">
												<script language="javascript">fnOrderBy('','desporce')</script>
											</td>
										...
										for ($i=0;$i<count($mDescuentos);$i++) {
											...
											<tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
												<td class="letra7"><a href = javascript:fnVer('<?php echo $mDescuentos[$i]['desidxxx']?>')><?php echo $mDescuentos[$i]['desidxxx'] ?></a></td>
												<td class="letra7"><?php echo $mDescuentos[$i]['descodxx'] ?></td>
												<td class="letra7"><?php echo $mDescuentos[$i]['seridxxx'] ?></td>
												<td class="letra7"><?php echo $mDescuentos[$i]['serdesxx'] ?></td>
												<td class="letra7"><?php echo $mDescuentos[$i]['fcoidxxx'] ?></td>
												<td class="letra7"><?php echo $mDescuentos[$i]['fcodesxx'] ?></td>
												<td class="letra7"><?php echo $mDescuentos[$i]['desporce'] ?></td>
												<td class="letra7"><?php echo $mDescuentos[$i]['regfcrex']." ".$mDescuentos[$i]['reghcrex'] ?></td>
												<td class="letra7"><?php echo $mDescuentos[$i]['regfmodx']." ".$mDescuentos[$i]['reghmodx'] ?></td>
												...
												<td Class="letra7" align="right"><input type="checkbox" name="oCheck"  value = "<?php echo count($mDescuentos) ?>"
													id="<?php echo $mDescuentos[$i]['desidxxx'].'~'.$mDescuentos[$i]['regestxx']?>"
