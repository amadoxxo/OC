        ...
        if ($nSwitch == 0) {
          ...
          if ($vSysStr['roldanlo_cuentas_anticipos_proveedor_financiacion_clientes'] != "") {
            ...
            if ($pvParametros['cliidxxx'] != "") {
              ...
              $qTramite .= "FROM $cAlfa.sys00121 ";
              $qTramite .= "WHERE cliidxxx = \"{$pvParametros['cliidxxx']}\"";
              $xTramite = mysql_query($qTramite,$xConexion01);
              if (mysql_num_rows($xTramite) > 0) {
                $cTraCliArray = [];
                while ($xCadena = mysql_fetch_array($xTramite)) {
                    $cTraCliArray[] = "(sucidxxx=\"{$xCadena['sucidxxx']}\" AND docidxxx=\"{$xCadena['docidxxx']}\" AND docsufxx=\"{$xCadena['docsufxx']}\")";
                }
                $cTraCli = implode(" OR ", $cTraCliArray);
              }
            }
            ...
            for ($iAno = $dAnioInicio; $iAno<=date('Y'); $iAno++) {
              ...
              if ($pvParametros['cliidxxx'] != "") {
                $qLoad .= "(";
                $qLoad .= "teridxxx=\"{$pvParametros['cliidxxx']}\" OR ";
                $qLoad .= "terid3xx=\"{$pvParametros['cliidxxx']}\" ";
                if ($cTraCli != "") {
                  $qLoad .= "OR $cTraCli";
                }
                $qLoad .= ") AND ";
              ...
              for ($i=0;$i<=$nRegistros;$i+=$nNumReg) {
                ...
                if ($pvParametros['cliidxxx'] != "") {
                  $qMovCon .= "(";
                  $qMovCon .= "teridxxx=\"{$pvParametros['cliidxxx']}\" OR ";
                  $qMovCon .= "terid3xx=\"{$pvParametros['cliidxxx']}\" ";
                  if ($cTraCli != "") {
                    $qMovCon .= "OR $cTraCli";
                  }
                  $qMovCon .= ") AND ";
          ...
          for ($iAno = $dAnioInicio; $iAno<=date('Y'); $iAno++) {
            ...
            $qLoad .= "$cAlfa.fcod$iAno.comfecxx BETWEEN \"{$vSysStr['financiero_ano_instalacion_modulo']}-01-01\" AND \"{$pvParametros['feccorte']}\" AND ";
            if ($pvParametros['cliidxxx'] != "") {
              $qLoad .= "$cAlfa.fcod$iAno.teridxxx = \"{$pvParametros['cliidxxx']}\" AND ";
            }
            ...
            for ($i=0;$i<=$nRegistros;$i+=$nNumReg) {
              ...
              $qMovCon .= "$cAlfa.fcod$iAno.comfecxx BETWEEN \"{$vSysStr['financiero_ano_instalacion_modulo']}-01-01\" AND \"{$pvParametros['feccorte']}\" AND ";
              if ($pvParametros['cliidxxx'] != "") {
                $qMovCon .= "$cAlfa.fcod$iAno.teridxxx = \"{$pvParametros['cliidxxx']}\" AND ";
              }
            ...
            $qLoad .= "$cAlfa.fcod$iAno.comfecxx BETWEEN \"{$vSysStr['financiero_ano_instalacion_modulo']}-01-01\" AND \"{$pvParametros['feccorte']}\" AND ";
            if ($pvParametros['cliidxxx'] != "") {
              $qLoad .= "$cAlfa.fcod$iAno.terid2xx = \"{$pvParametros['cliidxxx']}\" AND ";
            }
            ...
            for ($i=0;$i<=$nRegistros;$i+=$nNumReg) {
              ...
              $qMovCon .= "$cAlfa.fcod$iAno.comfecxx BETWEEN \"{$vSysStr['financiero_ano_instalacion_modulo']}-01-01\" AND \"{$pvParametros['feccorte']}\" AND ";
              if ($pvParametros['cliidxxx'] != "") {
                $qMovCon .= "$cAlfa.fcod$iAno.terid2xx = \"{$pvParametros['cliidxxx']}\" AND ";
              }
