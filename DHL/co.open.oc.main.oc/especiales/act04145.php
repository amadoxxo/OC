    ...
    function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
      ...
      if ($nSwitch == 0) {
        ...
        if ($mdb){
          ...
          $qInsert .= "modidxxx, proidxxx, menidxxx, menformx, menopcxx, menordxx, mendesxx, menimgon, menimgof, menalcxx, mentipxx, regfecxx, regmodxx, reghorxx, regestxx";
          $qInsert .= ") VALUES (";
          $qInsert .= "\"4\", \"327\", \"2\", \"frcrfnue.php\", \"EDITAR\", \"00002\", \"Editar Columna\", \"\", \"\", \"\", \"\", NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
          ...
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe da acceso a la opcion de Editar Conceptos Reporte Facturacion DHL en el modulo de Proyectos Especiales".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
          } else {
            $nSwitch = 1;
          ...
          $qInsert .= "usridxxx, modidxxx, proidxxx, menidxxx, regfecxx, regmodxx, reghorxx, regestxx";
          $qInsert .= ") VALUES (";
          $qInsert .= "\"ADMIN\", \"4\", \"327\", \"2\", NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe da acceso a la opcion de Borrar Conceptos Reporte Facturacion DHL en el modulo de Proyectos Especiales".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
          } else {
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError al dar acceso a la parametrica de Conceptos Reporte Facturacion DHL";
