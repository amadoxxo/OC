<?php
  ...
  define("_FILELOG_","act04264.log"); // Archivo de LOG
    ...
    function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
      ...
      if ($nSwitch == 0) {
        ...
        if ($mdb){
          ...
          $qVarSist  = "SELECT ";
          $qVarSist .= "$pdb.sys00002.strvlrxx ";
          $qVarSist .= "FROM $pdb.sys00002 ";
          $qVarSist .= "WHERE ";
          $qVarSist .= "$pdb.sys00002.stridxxx = \"logistica_ano_instalacion_modulo\" AND ";
          $qVarSist .= "$pdb.sys00002.regestxx = \"ACTIVO\" ";
          $qVarSist .= "LIMIT 0,1 ";
          $xVarSist = mysql_query($qVarSist,_CONEXION_);
          // echo $qVarSist.'~'.mysql_num_rows($xVarSist)."\n";
          if (mysql_num_rows($xVarSist) > 0) {
            $vVarSist = mysql_fetch_array($xVarSist);

            for ($Anio = $vVarSist['strvlrxx']; $Anio<=Date('Y'); $Anio++) {
              //Campo Anexos para la tabla lmca
              $qAlter  = "ALTER TABLE lmca$Anio ADD mifanexx VARCHAR(2) NULL COMMENT \"Documento Tiene Anexos (SI/NO)\" AFTER miffhaxx;";
              $xAlter  = mysql_query($qAlter,_CONEXION_);
              if ($xAlter) {
                $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Creo el nuevo campo para la tabla lmca$Anio en: ".$pdb;
                system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
              } else {
                $nSwitch = 1;
                $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError al crear el nuevo campo para la tabla lmca$Anio en: ".$pdb;
                system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
              }

              //Campo Anexos para la tabla lcca
              $qAlter  = "ALTER TABLE lcca$Anio ADD ceranexx VARCHAR(2) NULL COMMENT \"Documento Tiene Anexos (SI/NO)\" AFTER cerfhaxx;";
              $xAlter  = mysql_query($qAlter,_CONEXION_);
              if ($xAlter) {
                $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Creo el nuevo campo para la tabla lcca$Anio en: ".$pdb;
                system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
              } else {
                $nSwitch = 1;
                $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError al crear el nuevo campo para la tabla lcca$Anio en: ".$pdb;
                system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
              }

              //Campo Anexos para la tabla lpca
              $qAlter  = "ALTER TABLE lpca$Anio ADD pedanexx VARCHAR(2) NULL COMMENT \"Documento Tiene Anexos (SI/NO)\" AFTER pedvlrxx;";
              $xAlter  = mysql_query($qAlter,_CONEXION_);
              if ($xAlter) {
                $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Creo el nuevo campo para la tabla lpca$Anio en: ".$pdb;
                system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
              } else {
                $nSwitch = 1;
                $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError al crear el nuevo campo para la tabla lpca$Anio en: ".$pdb;
                system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
              }
            }
          }
