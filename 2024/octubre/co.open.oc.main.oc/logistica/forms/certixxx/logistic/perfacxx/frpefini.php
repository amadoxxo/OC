<?php
  namespace openComex;
/**
 * Tracking Periodicidad de Facturación
 * --- Este programa permite realizar consultas rapidas Periodicidad de Facturación que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xPfaId) {
        var cPathUrl = "frpefnue.php?cPfaId="+xPfaId;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Periodicidad de Facturacion;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frpefnue.php?cPfaId="+mMatriz[0];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Periodicidad de Facturacion;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de el Periodicidad de Facturacion["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
                  document.forms['frestado']['cPfaId'].value=mMatriz[0];
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frpefgra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cPfaId" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frpefini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mPerFac = array();
        $qPerFac  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qPerFac .= "$cAlfa.lpar0005.pfaidxxx, ";
        $qPerFac .= "$cAlfa.lpar0005.pfadesxx, ";
        $qPerFac .= "$cAlfa.lpar0005.regusrxx, "; // Código Usuario
        $qPerFac .= "$cAlfa.lpar0005.regfcrex, "; // Fecha de creación
        $qPerFac .= "$cAlfa.lpar0005.reghcrex, "; // Hora de creación
        $qPerFac .= "$cAlfa.lpar0005.regfmodx, "; // Fecha de modificación
        $qPerFac .= "$cAlfa.lpar0005.reghmodx, "; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qPerFac .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qPerFac .= "$cAlfa.lpar0005.regestxx ";
        $qPerFac .= "FROM $cAlfa.lpar0005 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qPerFac .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0005.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qPerFac .= "WHERE ";
          $qPerFac .= "$cAlfa.lpar0005.pfaidxxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qPerFac .= "$cAlfa.lpar0005.pfadesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qPerFac .= "$cAlfa.lpar0005.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qPerFac .= "$cAlfa.lpar0005.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qPerFac .= "$cAlfa.lpar0005.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qPerFac .= "$cAlfa.lpar0005.regusrxx IN ($cNombreSearch) OR ";
          }
          $qPerFac .= "$cAlfa.lpar0005.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        $qPerFac .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xPerFac = mysql_query($qPerFac, $xConexion01, true, $cIdCountRow);
        //echo $qPerFac." ~ ".mysql_num_rows($xPerFac);
        ...
        while ($xRDC = mysql_fetch_array($xPerFac)) {
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="13%">
                        <a href = "javascript:fnOrder_By('onclick','pfaidxxx');" title="Ordenar">ID</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "pfaidxxx">
                        <input type = "hidden" name = "pfaidxxx" value = "<?php echo $_POST['pfaidxxx'] ?>" id = "pfaidxxx">
                        <script language="javascript">fnOrder_By('','pfaidxxx')</script>
                      </td>
                      <td class="name" width="40%">
                        <a href = "javascript:fnOrder_By('onclick','pfadesxx');" title="Ordenar">Periodicidad de Facturaci&oacute;n</a>&nbsp;
                        ...
                        <script language="javascript">fnOrder_By('','pfadesxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mPerFac) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mPerFac);$i++) {
                        if ($i < count($mPerFac)) { // Para Controlar el Error
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mPerFac[$i]['pfaidxxx']?>')>
                                                        <?php echo $mPerFac[$i]['pfaidxxx']?>
                            </a>
                          <td class="letra7" width="40%"><?php echo $mPerFac[$i]['pfadesxx'] ?></td>
                          <td class="letra7" width="18%"><?php echo substr($mPerFac[$i]['usrnomxx'],0,20) ?></td>
                          <td class="letra7" width="07%"><?php echo $mPerFac[$i]['regfcrex'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mPerFac[$i]['reghcrex'] ?></td>
                          <td class="letra7" width="07%"><?php echo $mPerFac[$i]['regfmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mPerFac[$i]['reghmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mPerFac[$i]['regestxx'] ?></td>
                          <td Class="letra7" width="02%" align="right">
                            <input type="checkbox" name="oCheck" value = "<?php echo count($mPerFac) ?>"
                            id="<?php echo $mPerFac[$i]['pfaidxxx'].'~'.
                                            $mPerFac[$i]['regestxx'] ?>"
                            onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mPerFac) ?>'">
                          </td>
                        </tr>
                        <?php $y++;
                        }
