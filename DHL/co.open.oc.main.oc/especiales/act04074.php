    ...
    function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
      ...
      if ($nSwitch == 0) {
        ...
        if ($mdb){
          ...
          $qCreate  = "CREATE TABLE zdex0011 ( ";
					$qCreate .= "tfvidxxx int(11) NOT NULL AUTO_INCREMENT COMMENT \"Id Texto Factura de Venta DHL\",";
					$qCreate .= "tfvtitxx varchar(150) NOT NULL COMMENT \"Titulo\",";
					$qCreate .= "tfvcontx text NOT NULL COMMENT \"Contenido\",";
					...
					if ($xCreate) {
						$cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Creo la Tabla zdex0011 en: ".$pdb;
						system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
					} else {
						$nSwitch = 1;
						$cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qCreate." ~ ".mysql_error(_CONEXION_)."\tError Al Crear la Tabla zdex0011 en: ".$pdb;
