...
/**
 * Tracking Servicio.
 * --- Este programa permite realizar consultas rapidas Servicio que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xSerSap) {
        var cPathUrl = "frsernue.php?cSerSap="+xSerSap;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Servicio;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frsernue.php?cSerSap="+mMatriz[0];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Servicio;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de el Servicio ["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frsergra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cSerSap" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frserini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mServix = array();
        $qServix  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qServix .= "$cAlfa.lpar0011.sersapxx, ";
        $qServix .= "$cAlfa.lpar0011.serdesxx, ";
        $qServix .= "$cAlfa.lpar0011.regusrxx, "; // Código Usuario
        $qServix .= "$cAlfa.lpar0011.regfcrex, "; // Fecha de creación
        $qServix .= "$cAlfa.lpar0011.reghcrex, "; // Hora de creación
        $qServix .= "$cAlfa.lpar0011.regfmodx, "; // Fecha de modificación
        $qServix .= "$cAlfa.lpar0011.reghmodx, "; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qServix .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qServix .= "$cAlfa.lpar0011.regestxx ";
        $qServix .= "FROM $cAlfa.lpar0011 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qServix .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0011.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qServix .= "WHERE ";
          $qServix .= "$cAlfa.lpar0011.sersapxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qServix .= "$cAlfa.lpar0011.serdesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qServix .= "$cAlfa.lpar0011.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qServix .= "$cAlfa.lpar0011.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qServix .= "$cAlfa.lpar0011.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qServix .= "$cAlfa.lpar0011.regusrxx IN ($cNombreSearch) OR ";
          }
          $qServix .= "$cAlfa.lpar0011.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        $qServix .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xServix = mysql_query($qServix, $xConexion01, true, $cIdCountRow);
        //echo $qServix." ~ ".mysql_num_rows($xServix);
        ...
        while ($xRDC = mysql_fetch_array($xServix)) {
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>Servicio (<?php echo $nRNR?>)</legend>
                <center>
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="13%">
                        <a href = "javascript:fnOrder_By('onclick','sersapxx');" title="Ordenar">Cod Sap</a>&nbsp;
                      ...
                      <td class="name" width="40%">
                        <a href = "javascript:fnOrder_By('onclick','serdesxx');" title="Ordenar">Servicio</a>&nbsp;
                        ...
                        <script language="javascript">fnOrder_By('','serdesxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mServix) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mServix);$i++) {
                        if ($i < count($mServix)) { // Para Controlar el Error
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mServix[$i]['sersapxx']?>')>
                                                        <?php echo $mServix[$i]['sersapxx']?>
                            </a>
                          <td class="letra7" width="40%"><?php echo $mServix[$i]['serdesxx'] ?></td>
                          <td class="letra7" width="18%"><?php echo substr($mServix[$i]['usrnomxx'],0,20) ?></td>
                          <td class="letra7" width="07%"><?php echo $mServix[$i]['regfcrex'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mServix[$i]['reghcrex'] ?></td>
                          <td class="letra7" width="07%"><?php echo $mServix[$i]['regfmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mServix[$i]['reghmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mServix[$i]['regestxx'] ?></td>
                          <td Class="letra7" width="02%" align="right">
                            <input type="checkbox" name="oCheck" value = "<?php echo count($mServix) ?>"
                            id="<?php echo $mServix[$i]['sersapxx'].'~'.
                                            $mServix[$i]['regestxx'] ?>"
                            onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mServix) ?>'">
                          </td>
                        </tr>
                        <?php $y++;
                        }
