 	    ...
 	    <?php
          ...
          $vLimSup = $vSysStr['system_rows_page_ini'];
        	...
        	$vUsrId = $_COOKIE['kUsrId'];
				...
				/**
				 * Realizo la consulta de las Facturas Autorizadas o no.
				 */
				if($cPerAno == ""){
				  $cPerAno = date('Y');
				}
		   	...
		   	$y=0;
				...
				$zSqlCab .= "$cAlfa.fcoc$cPerAno.ccoidxxx, ";
				$zSqlCab .= "$cAlfa.fcoc$cPerAno.teridxxx, ";
				$zSqlCab .= "$cAlfa.fcoc$cPerAno.comfecxx ";
				$zSqlCab .= "FROM $cAlfa.fcoc$cPerAno ";
				$zSqlCab .= "WHERE ";
				$zSqlCab .= "$cAlfa.fcoc$cPerAno.comidxxx = \"F\" AND ";
				$zSqlCab .= "$cAlfa.fcoc$cPerAno.regestxx = \"ACTIVO\" AND ";
				$zSqlCab .= "$cAlfa.fcoc$cPerAno.comprnxx = \"\" ";
				$zSqlCab .= "ORDER BY CONVERT($cAlfa.fcoc$cPerAno.comcscxx,signed) ASC ";
				...
				for ($i=0;$i<count($zMatrizTmp);$i++) {
				  //$zMatrizTmp[$i]['SERIDXXX'] = intval($zMatrizTmp[$i]['SERIDXXX']);
					...
					//f_Mensaje(__FILE__,__LINE__,$zSqlPto);
        ...
        <script language="javascript">
       	...
       	<table width="95%" cellspacing="0" cellpadding="0" border="0">
         	<tr>
	       	  <td>
				      <fieldset>
   			        <legend>Autorizaciones Realizadas(<?php echo count($zMatrizTra) ?>)</legend>
     	       		<center>
