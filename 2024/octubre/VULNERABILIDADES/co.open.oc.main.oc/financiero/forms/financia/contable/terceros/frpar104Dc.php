<?php 
  namespace openComex;
	...
	$qTblCom = "SELECT TABLE_COMMENT FROM $cOmega.TABLES WHERE TABLE_SCHEMA = \"$cAlfa\" AND TABLE_NAME = \"fpar0117\" LIMIT 0,1";
														...
														<?php while ($mPaiDes = mysql_fetch_array($xPaiDes)) {
															if (mysql_num_rows($xPaiDes) > 1) { ?>
																<tr>
																	<td width = "050" class= "name">
																		<a href = "javascript:window.opener.document.forms['frgrm']['cPaiIdDc'].value  ='<?php echo $mPaiDes['paiidxxx']?>';
																													window.opener.document.forms['frgrm']['cPaiDesDc'].value ='<?php echo $mPaiDes['paidesxx']?>';
															...
															<?php	} else { ?>
																<script languaje="javascript">
																	window.opener.document.forms['frgrm']['cPaiIdDc'].value  = '<?php echo $mPaiDes['paiidxxx'] ?>';																
																	window.opener.document.forms['frgrm']['cPaiDesDc'].value = '<?php echo $mPaiDes['paidesxx'] ?>';
																	close();
																</script>																	
															<?php }
	  								...
	  								case "VALID":
	  									...
	  									if ($xPaiDes && mysql_num_rows($xPaiDes) > 0) {
	  										while ($mPaiDes = mysql_fetch_array($xPaiDes)) { ?>
													<script languaje = "javascript">
														parent.fmwork.document.forms['frgrm']['cPaiIdDc'].value  = '<?php echo $mPaiDes['paiidxxx'] ?>';													
														parent.fmwork.document.forms['frgrm']['cPaiDesDc'].value = '<?php echo $mPaiDes['paidesxx'] ?>';
														close();
													</script>
	      	      				<?php break;
	  										}
