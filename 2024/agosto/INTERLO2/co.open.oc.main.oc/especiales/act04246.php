<?php
  ...
  for ($i=0;$i<count($aconn);$i++){
    $vdb  = $aconn[$i]['DB'];
    $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tCreacion Campos (Criterios) Tercero openComex: ".$vdb;
      ...
      if ($nSwitch == 0) {
        ...
        if ($mdb){
          ...
          if ($xAlter) {
            ...
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError al agregar la columna CLICPLCR a la tabla SIAI0150 : ".$pdb;
          ...
          if ($xAlter) {
            ...
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError al agregar la columna CLICCRMA a la tabla SIAI0150 : ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
          }
          ...
          if ($xAlter) {
            ...
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError al agregar la columna CLIPPLCR a la tabla SIAI0150 : ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
          }
          ...
          if ($xAlter) {
            ...
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError al agregar la columna CLIPCRMA a la tabla SIAI0150 : ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
          }
