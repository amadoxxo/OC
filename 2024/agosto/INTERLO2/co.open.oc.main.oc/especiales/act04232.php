<?php
  ...
  for ($i=0;$i<count($aconn);$i++){
    $vdb  = $aconn[$i]['DB'];
    $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tCreación de las Tablas Para el Módulo de Logistica Fase 3: ".$vdb;
    ...
    function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
      ...
      if ($nSwitch == 0) {
        ...
        if ($mdb){
          ...
          ###################################
          //Agregacion de columna a la tabla fpar0119
          ###################################
          $qAlter  = "ALTER TABLE fpar0119 ";
          $qAlter .= "ADD COLUMN ctoapcon VARCHAR(2) NULL COMMENT \"Aplica como concepto de Carta Bancaria en los Ajustes\" ";
          $qAlter .= "AFTER ctocwccx";
          $xAlter  = mysql_query($qAlter,_CONEXION_);   
          if ($xAlter) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe agrego la columna ctoapcon a la tabla fpar0119 correctamente : ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
          } else {
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qAlter." ~ ".mysql_error(_CONEXION_)."\tError al agregar la columna ctoapcon a la tabla fpar0119 : ".$pdb;
