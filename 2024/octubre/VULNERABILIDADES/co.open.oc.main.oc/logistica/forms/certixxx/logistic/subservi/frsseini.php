<?php
  namespace openComex;
/**
 * Tracking Sub Servicios.
 * --- Descripcion: Este programa permite realizar consultas rapidas Sub Servicios que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xSerSap,xSSerId) {
        var cPathUrl = "frssenue.php?cSerSap="+xSerSap+"&cSSerId="+xSSerId;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Sub Servicios;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frssenue.php?cSerSap="+mMatriz[0]+"&cSSerId="+mMatriz[1];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Sub Servicios;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado del Sub Servicios ["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
                  document.forms['frestado']['cSerSap'].value=mMatriz[0];
                  document.forms['frestado']['cSSerId'].value=mMatriz[1];
      ...
      function fnGenerarExcel() {
        parent.fmpro.location = "frsseprn.php";
      }

    </script>
  </head>
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frssegra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cSerSap" value = "">
      <input type = "hidden" name = "cSSerId" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frsseini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mSubSer = array();
        $qSubSer  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qSubSer .= "$cAlfa.lpar0012.sersapxx, ";
        $qSubSer .= "$cAlfa.lpar0012.subidxxx, ";
        $qSubSer .= "$cAlfa.lpar0012.subdesxx, ";
        $qSubSer .= "$cAlfa.lpar0011.serdesxx, ";
        $qSubSer .= "$cAlfa.lpar0012.regusrxx, "; // Código Usuario
        $qSubSer .= "$cAlfa.lpar0012.regfcrex, "; // Fecha de creación
        $qSubSer .= "$cAlfa.lpar0012.reghcrex, "; // Hora de creación
        $qSubSer .= "$cAlfa.lpar0012.regfmodx, "; // Fecha de modificación
        $qSubSer .= "$cAlfa.lpar0012.reghmodx, "; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qSubSer .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qSubSer .= "$cAlfa.lpar0012.regestxx ";
        $qSubSer .= "FROM $cAlfa.lpar0012 INNER JOIN $cAlfa.lpar0011 ON lpar0012.sersapxx = lpar0011.sersapxx ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qSubSer .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0012.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qSubSer .= "WHERE ";
          $qSubSer .= "$cAlfa.lpar0012.subidxxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qSubSer .= "$cAlfa.lpar0012.subdesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qSubSer .= "$cAlfa.lpar0012.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qSubSer .= "$cAlfa.lpar0012.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qSubSer .= "$cAlfa.lpar0012.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qSubSer .= "$cAlfa.lpar0012.regusrxx IN ($cNombreSearch) OR ";
          }
          $qSubSer .= "$cAlfa.lpar0012.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        if (strlen($cOrderBy)>0) {
          ...
          $cOrderBy = "ORDER BY regfmodx DESC, reghcrex DESC ";
        }
        //// FIN CODIGO NUEVO PARA ORDER BY
        $qSubSer .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xSubSer = mysql_query($qSubSer, $xConexion01, true, $cIdCountRow);
        //echo $qSubSer." ~ ".mysql_num_rows($xSubSer);
        ...
        while ($xRDC = mysql_fetch_array($xSubSer)) {
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>SubServicios (<?php echo $nRNR?>)</legend>
                <center>
                        ...
                        <?php
                          ...
                          while ($mBotAcc = mysql_fetch_array($xBotAcc)) {
                            switch ($mBotAcc['menopcxx']) {
                              ...
                              <?php break;
                              case "EXCEL": ?>
                                <img src = "<?php echo $cPlesk_Skin_Directory_Logistic ?>/excel_icon.gif" onClick = "javascript:fnGenerarExcel('<?php echo $mBotAcc['menopcxx'] ?>')" style = "cursor:pointer" title="<?php echo $mBotAcc['mendesxx'] ?>">
                              <?php break;
                            }
                          }
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="10%">
                        <a href = "javascript:fnOrder_By('onclick','subidxxx');" title="Ordenar">ID</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic?>/spacer.png" border="0" width="11" height="9" title = "" id = "subidxxx">
                        <input type = "hidden" name = "subidxxx" value = "<?php echo $_POST['subidxxx'] ?>" id = "subidxxx">
                        <script language="javascript">fnOrder_By('','subidxxx')</script>
                      </td>
                      <td class="name" width="21%">
                        <a href = "javascript:fnOrder_By('onclick','subdesxx');" title="Ordenar">Subservicio</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic?>/spacer.png" border="0" width="11" height="9" title = "" id = "subdesxx">
                        <input type = "hidden" name = "subdesxx" value = "<?php echo $_POST['subdesxx'] ?>" id = "subdesxx">
                        <script language="javascript">fnOrder_By('','subdesxx')</script>
                      </td>
                      <td class="name" width="22%">
                        <a href = "javascript:fnOrder_By('onclick','sersapxx');" title="Ordenar">Servicio</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic?>/spacer.png" border="0" width="11" height="9" title = "" id = "sersapxx">
                        <input type = "hidden" name = "sersapxx" value = "<?php echo $_POST['sersapxx'] ?>" id = "sersapxx">
                        <script language="javascript">fnOrder_By('','sersapxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mSubSer) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mSubSer);$i++) {
                        if ($i < count($mSubSer)) { // Para Controlar el Error
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mSubSer[$i]['sersapxx']?>','<?php echo $mSubSer[$i]['subidxxx']?>')>
                                                        <?php echo $mSubSer[$i]['subidxxx']?>
                            </a>
                          <td class="letra7" width="20%"><?php echo $mSubSer[$i]['subdesxx'] ?></td>
                          <td class="letra7" width="20%"><?php echo $mSubSer[$i]['serdesxx'] ?></td>
                          <td class="letra7" width="18%"><?php echo substr($mSubSer[$i]['usrnomxx'],0,20) ?></td>
                          <td class="letra7" width="07%"><?php echo $mSubSer[$i]['regfcrex'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mSubSer[$i]['reghcrex'] ?></td>
                          <td class="letra7" width="07%"><?php echo $mSubSer[$i]['regfmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mSubSer[$i]['reghmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mSubSer[$i]['regestxx'] ?></td>
                          <td Class="letra7" width="02%" align="right">
                            <input type="checkbox" name="oCheck" value = "<?php echo count($mSubSer) ?>"
                            id="<?php echo $mSubSer[$i]['sersapxx'].'~'.
                                            $mSubSer[$i]['subidxxx'].'~'.
                                            $mSubSer[$i]['regestxx'] ?>"
                            onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mSubSer) ?>'">
                          </td>
                        </tr>
                        <?php $y++;
                        }
