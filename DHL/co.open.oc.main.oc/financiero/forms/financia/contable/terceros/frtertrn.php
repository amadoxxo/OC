<?php
	/**
	 * Carga loa Tributos en la ficha de Terceros.
...
<html>
	<head>
		...
		function fnGuardar(){
			...
			if (cadeni.length > 1)	{
				var cRuta = "frtertrg.php?cTerId=<?php echo $cTerId ?>&cCadena="+cadeni+"&cCliTri="+window.opener.document.forms['frgrm']['cCliTri'].value+"&tipsave=5";
			...
			if(http_request.readyState==1){          
				...
				if (http_request.status == 200) {
					if(http_request.responseText!=""){
						...
						if (mRetorno[0] == "true") {
							window.opener.document.forms['frgrm']['cCliTri'].value = mRetorno[1];
							window.opener.fnCargarTributo();                
							window.close();
						} else {
							alert(mRetorno[1]);
						}
						...
						<fieldset>
	  					...
	  					<form name = "frgrm" action = "" method = "post" target = "fmpro">
	  						...
	  						$qTributo .= "regestxx ";																								
								...
								$qTributo .= "WHERE ";
								$qTributo .= "regestxx = \"ACTIVO\" ORDER BY triidxxx";
								...
								if (mysql_num_rows($xTributo) > 0) {
					    			...
					    			<table cellspacing = "0" cellpadding = "1" border = "1" width = "450">
											...
											while ($xRTR = mysql_fetch_array($xTributo)) {
											  $cvb  = 0;
                        if (in_array($xRTR['triidxxx'],$mTributo) == true) {
                         	$cvb = 1;
                        }							          
								          ...
								          if($y % 2 == 0) {
								          	$zColor = "{$vSysStr['system_row_par_color_ini']}";
											   	...
											   	<tr bgcolor = "<?php echo $zColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $zColor ?>')">
														<td style='width: 020px' Class = 'letra7'><center><input type = 'checkbox' style = 'width:20' name = '<?php echo $xRTR['triidxxx'] ?>' onClick ='javascript:f_Co(this)'></center></td>
														<td style='width: 060px' Class = 'letra7'><?php echo $xRTR['triidxxx'] ?></td>
														<td style='width: 240px' Class = 'letra7'><?php echo substr($xRTR['tridesxx'],0,45) ?></td>
														<td style='width: 080px' Class = 'letra7'><?php echo $xRTR['triaplxx'] ?></td>
