<?php
  /**
   * Nota Ajuste Documento Soporte Autofactura.
   * --- Descripcion: Permite Imprimir el Formato Estandar de Nota Ajuste Documento Soporte Autofactura.
  ...
  // Traigo datos del documento soporte referencia
  $qDocSoporte  = "SELECT  ";
  $qDocSoporte .= "resprexx, ";
  $qDocSoporte .= "comcscxx, ";
  $qDocSoporte .= "comptecu, ";
  $qDocSoporte .= "comfecxx ";
  $qDocSoporte .= "FROM $cAlfa.fdsc{$vComObs2[6]} ";
  $qDocSoporte .= "WHERE ";
  $qDocSoporte .= "comidxxx = \"{$vComObs2[7]}\" AND ";
  $qDocSoporte .= "comcodxx = \"{$vComObs2[8]}\" AND ";
  $qDocSoporte .= "comcscxx = \"{$vComObs2[9]}\" AND ";
  $qDocSoporte .= "comcsc2x = \"{$vComObs2[9]}\" LIMIT 0,1 ";
  $xDocSoporte  = mysql_query($qDocSoporte,$xConexion01);
  // f_Mensaje(__FILE__,__LINE__,$qDocSoporte."~".mysql_num_rows($xDocSoporte));
  $vDocSoporte = mysql_fetch_array($xDocSoporte);
  $cDocRef     = $vDocSoporte['resprexx'].$vDocSoporte['comcscxx'];
  $dFecDocRef  = $vDocSoporte['comfecxx'];
  $cCufeDocRef = $vDocSoporte['comptecu'];
  ...
  class PDF extends FPDF {
    function Header() {
      ...
      global $vFacturar; global $vCiuFact;   global $vResDat; global $cDocRef;  global $dFecDocRef; global $cCufeDocRef;
      ...
      $this->SetFont("verdanab", '', 7);
      $this->setXY($posx + 65, $posy + 30);
      $this->MultiCell(130, 4, utf8_decode("NOTA DE AJUSTE DEL DOCUMENTO SOPORTE EN ADQUISICIONES EFECTUADAS A NO OBLIGADOS A EXPEDIR FACTURA O DOCUMENTO EQUIVALENTE No. " . $vResDat['resprexx']." ".$vComCab['comcscxx']), 0, 'R');
      $this->Ln(1);
      $this->SetFont("verdana", '', 7);
      $this->setX($posx + 66);
      ...
      $this->Cell(55, 6, utf8_decode("Fecha Documento Referencia"), "B", 0, 'L', true);
      $this->Cell(100, 6, utf8_decode("CUFE"), "B", 0, 'L', true);
      
      $this->SetFillColor(255, 255, 255);
      $this->SetFont('verdana', '', 7);
      $this->setXY($posx, $posy+6);
      $this->Cell(40, 6, utf8_decode($cDocRef), "", 0, 'L');
      $this->Cell(55, 6, utf8_decode($dFecDocRef), "", 0, 'L');
      $this->MultiCell(100, 3, $cCufeDocRef, 0, 'L');
      $posy += 17;
    ...
    function Footer(){
      ...
      $this->Ln(4);
      $this->setX($posx);
      $this->Cell(100,4,utf8_decode("Representación gráfica de la nota de ajuste de documento soporte electrónico: "),0,0,'L');
      $this->Ln(4);
      $this->setX($posx);
  ...
  // $mDataItem = array_merge($mDataItem, $mDataItem, $mDataItem, $mDataItem);
