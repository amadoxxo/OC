...
<html>
	<head>
		...
		<script language="javascript">
		  ...
		  function f_EnabledCombos() { // Devuelvo al Formulario que Me Llama los Datos de la Aplicacion
		    //alert("Entre1");
				...
				document.getElementById('rTipSecu').disabled  = false;
				document.getElementById('rTipPrin').disabled  = false;
		  }
		  ...
		  function f_DisabledCombos() { // Devuelvo al Formulario que Me Llama los Datos de la Aplicacion
		    //alert("Entre2");
				...
				document.getElementById('rTipSecu').disabled  = true;
				document.getElementById('rTipPrin').disabled  = true;
		  }
    	...
    	function f_Links(xLink,xSwitch,xIteration) {
				...
				switch (xLink){
				  case "cComId":
			     if (xSwitch == "VALID") {
             var zRuta  = "frres117.php?gWhat=VALID&gFunction=cComId&gComId="+
							             document.forms['frgrm']['cComId'].value.toUpperCase();
						 parent.fmpro.location = zRuta;
						   ...
						   var zRuta   = "frres117.php?gWhat=WINDOW&gFunction=cComId&gComId="+
							                 document.forms['frgrm']['cComId'].value.toUpperCase();
				  ...
				  case "cSucId":
						if (xSwitch == "VALID") {
							var zRuta  = "frpar008.php?gWhat=VALID&gFunction=cSucId&cSucId="+document.frgrm.cSucId.value.toUpperCase()+"";
							parent.fmpro.location = zRuta;
						} else {
		  				var zNx     = (zX-600)/2;
							var zNy     = (zY-250)/2;
							var zWinPro = 'width=600,scrollbars=1,height=250,left='+zNx+',top='+zNy;
							var zRuta   = "frpar008.php?gWhat=WINDOW&gFunction=cSucId&cSucId="+document.frgrm.cSucId.value.toUpperCase()+"";
							zWindow = window.open(zRuta,"zWindow",zWinPro);
					  	zWindow.focus();
						}
				  break;
	...
	<body topmargin = 0 leftmargin = 0 margnwidth = 0 marginheight = 0 style = 'margin-right:0'>
			...
			<table border ="0" cellpadding="0" cellspacing="0" width="300">
				<tr>
					<td>
				  	<fieldset>
					  	<legend>Nuevo <?php echo $_COOKIE['kProDes'] ?></legend>
						 	<form name = 'frgrm' action = 'frresgra.php' method = 'post' target='fmpro'>
                    ...
                    <?php $nCol = f_Format_Cols(20);
  							 		echo $nCol;?>
  							    <tr>
  	                  <td class="name" colspan = "20"><br>Tipo de Resolucion &nbsp;:
  	                    <input type="radio" name="rTipo" id="rTipPrin" value="PRINCIPAL" checked>Principal
  			                <input type="radio" name="rTipo" id="rTipSecu" value="SECUNDARIA">Secundaria<br>
  			              </td>
  	                </tr>
 							 		</table>
     							<table border = '0' cellpadding = '0' cellspacing = '0' width='300'>
                      ...
                      <td Class = "name" colspan = "10">Resolucion<br>
  										...
  										<td Class = "name" colspan = "3">Prefijo<br>
  										  ...
  										  <input type = "hidden" name = "cResPreAnt">
  									  </td>
  										<td Class = "name" colspan = "7">Clase<br>
  										  <input type = "text" Class = "letra" name = "cComCla" style = "width:140" maxlength="13"  readonly>
  									  </td>
										</tr>
  								</table>
  								<table border = "0" cellpadding = "0" cellspacing = "0" width="400">
		...
		switch ($_COOKIE['kModo']) {
			case "NUEVO":
				...
				<script languaje = "javascript">
				  document.forms['frgrm']['cComCla'].value = "AUTORIZACION";
  		  ...
  		  $cSqlRes .= "WHERE ";
  		  $cSqlRes .= "resclaxx = \"AUTORIZACION\" AND ";
      ...
      while ($zRRes = mysql_fetch_array($zCrsRes)){ ?>
        <script language = "javascript">
          <?php if($zRRes['restipxx']== "PRINCIPAL"){?>
            document.getElementById('rTipPrin').checked= true;
            document.getElementById('rTipSecu').checked= false;
          <?php }?>
          <?php if($zRRes['restipxx']== "SECUNDARIA"){?>
            document.getElementById('rTipSecu').checked= true;
            document.getElementById('rTipPrin').checked= false;
          <?php }?>
          document.forms['frgrm']['cResId'].value     = "<?php echo $zRRes['residxxx'] ?>";
          document.forms['frgrm']['cResPre'].value    = "<?php echo $zRRes['resprexx'] ?>";
          document.forms['frgrm']['cResPreAnt'].value = "<?php echo $zRRes['resprexx'] ?>";
          document.forms['frgrm']['cComCla'].value    = "<?php echo $zRRes['resclaxx'] ?>";
            ...
            for($i=0; $i<count($zComPro); $i++){
              if($zComPro[$i]!=""){
                ...
                if($zRCom['comidxxx']==$zComCad[0] and $zRCom['comcodxx']==$zComCad[1]) {
                  ?>
                  <script>
                  	if (document.frgrm.gIteration.value > 1) {
