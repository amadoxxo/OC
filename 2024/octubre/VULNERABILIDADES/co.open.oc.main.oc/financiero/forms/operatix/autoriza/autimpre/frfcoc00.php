						...
						<fieldset>
	  					...
	  					<form name = "frgrm" action = "" method = "post" target = "fmpro">
	  						<?php
	  							switch ($gWhat) {
	  								case "WINDOW":
	  									//f_Mensaje(__FILE__,__LINE__,"Hola window");
	  									##Traigo Datos de la factura ## 						  		
											...
											$qFacDat .= "$cAlfa.fpar0116.ccodesxx ";
											$qFacDat .= "FROM $cAlfa.fcoc$cPerAno ";
											$qFacDat .= "LEFT JOIN $cAlfa.fpar0116 ON $cAlfa.fcoc$cPerAno.ccoidxxx  = $cAlfa.fpar0116.ccoidxxx ";
											// $qFacDat .= "WHERE ";
											// $qFacDat .= "$cAlfa.fcoc$cPerAno.comidxxx = \"F\" AND ";
											// $qFacDat .= "$cAlfa.fcoc$cPerAno.comcscxx LIKE \"%$cComCsc%\" AND ";											
											// $qFacDat .= "$cAlfa.fcoc$cPerAno.regestxx != \"INACTIVO\" AND ";
											// $qFacDat .= "$cAlfa.fcoc$cPerAno.comprnxx = \"IMPRESO\" ";											
											$qFacDat .= "ORDER BY $cAlfa.fcoc$cPerAno.comfecxx ASC ";
											$xFacDat = f_MySql("SELECT","",$qFacDat,$xConexion01,"");	
											//f_Mensaje(__FILE__,__LINE__,$qFacDat);
																...
																<tr>
																	...
																	<td width = "100" class= "name">
																		<a href = "javascript:window.opener.document.forms['frgrm']['cComId'].value  ='<?php echo $mFacDat['comidxxx']?>';
																													...
																													window.opener.document.forms['frgrm']['cCcoDes'].value ='<?php echo $mFacDat['ccodesxx']?>';
	  								...
	  								case "VALID":
	  									//f_Mensaje(__FILE__,__LINE__,"Hola.....");
											...
											$qFacDat .= "WHERE ";
											$qFacDat .= "$cAlfa.fcoc$cPerAno.comidxxx = \"F\" AND ";
											$qFacDat .= "$cAlfa.fcoc$cPerAno.comcscxx = \"$cComCsc\" AND ";			//\"%$cComCsc%\"								
											$qFacDat .= "$cAlfa.fcoc$cPerAno.regestxx != \"INACTIVO\" AND ";
											$qFacDat .= "$cAlfa.fcoc$cPerAno.comprnxx = \"IMPRESO\" ";
											$qFacDat .= "ORDER BY $cAlfa.fcoc$cPerAno.comfecxx ASC ";
											$xFacDat = f_MySql("SELECT","",$qFacDat,$xConexion01,"");		
	  										...
	  										while ($mFacDat = mysql_fetch_array($xFacDat)) { 
	  											...
	  											?>
													<script language = "javascript">
														...
														parent.fmwork.document.forms['frgrm']['cCcoDes'].value = '<?php echo $mFacDat['ccodesxx'] ?>';
														close();
													</script>
	      	      				<?php break;
	  										}
