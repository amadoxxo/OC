<?php
  namespace openComex;
/**
 * Tracking Organizacion de Ventas.
 * --- Descripcion: Este programa permite realizar consultas rapidas Organizacion de Ventas que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xOrvSap) {
        var cPathUrl = "frorvnue.php?cOrvSap="+xOrvSap;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Organizacion de Ventas;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frorvnue.php?cOrvSap="+mMatriz[0];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de la Organizacion de Ventas ["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frorvgra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cOrvSap" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frorvini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $qOrgVen .= "$cAlfa.lpar0001.orvsapxx, ";
        $qOrgVen .= "$cAlfa.lpar0001.orvdesxx, ";
        $qOrgVen .= "$cAlfa.lpar0001.regusrxx, "; // Código Usuario
        $qOrgVen .= "$cAlfa.lpar0001.regfcrex, "; // Fecha de creación
        $qOrgVen .= "$cAlfa.lpar0001.reghcrex, "; // Hora de creación
        $qOrgVen .= "$cAlfa.lpar0001.regfmodx, "; // Fecha de modificación
        $qOrgVen .= "$cAlfa.lpar0001.reghmodx, "; // Hora de modificación
        ...
        $qOrgVen .= "$cAlfa.lpar0001.regestxx ";
        $qOrgVen .= "FROM $cAlfa.lpar0001 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qOrgVen .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0001.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qOrgVen .= "WHERE ";
          $qOrgVen .= "$cAlfa.lpar0001.orvsapxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qOrgVen .= "$cAlfa.lpar0001.orvdesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qOrgVen .= "$cAlfa.lpar0001.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qOrgVen .= "$cAlfa.lpar0001.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qOrgVen .= "$cAlfa.lpar0001.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qOrgVen .= "$cAlfa.lpar0001.regusrxx IN ($cNombreSearch) OR ";
          }
          $qOrgVen .= "$cAlfa.lpar0001.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>Organizacion de Ventas (<?php echo $nRNR?>)</legend>
                <center>
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      ...
                      <td class="name" width="40%">
                        <a href = "javascript:fnOrder_By('onclick','orvdesxx');" title="Ordenar">Organizaci&oacute;n de Ventas</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "orvdesxx">
                        <input type = "hidden" name = "orvdesxx" value = "<?php echo $_POST['orvdesxx'] ?>" id = "orvdesxx">
                        <script language="javascript">fnOrder_By('','orvdesxx')</script>
                      </td>
                      ...
                      <?php for ($i=0;$i<count($mOrgVen);$i++) {
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mOrgVen[$i]['orvsapxx']?>')>
                                                        <?php echo $mOrgVen[$i]['orvsapxx']?>
                            </a>
                          <td class="letra7" width="40%"><?php echo $mOrgVen[$i]['orvdesxx'] ?></td>
