    ...
    if (strlen($mPrn[$nn]) > 0) {
      ...
      $qCodDat .= "LEFT JOIN $cAlfa.SIAI0150 ON $cAlfa.fcod$cAno.teridxxx = $cAlfa.SIAI0150.CLIIDXXX ";
          ...
          // # DE PAGINAS //
	       ...
	       $pdf->SetFont('otfon1','',12);
				 $pdf->setXY(15,$posy4);
				 $pdf->Cell(150,3,"Cliente :".''. $vCocDat['CLINOMXX']);
				 $pdf->setXY(135,$posy4);
				 $pdf->Cell(150,3,"Consecutivo: ".(($vCocDat['comcsc3x'] != "") ? $vCocDat['comcsc3x'] : $vCocDat['comcsc2x']));
				 ...
				 $pdf->setXY(107,07);
				 $pdf->SetFont('otfon1','',12);
				 //$pdf->Cell(20,3,date(' Y    m    d'),0,0,"R");
				 $vMtzTar = explode("-",$vCocDat['comfecve']);
				 $pdf->Cell(20,3,$vMtzTar[0].'     '.$vMtzTar[1].'     '.$vMtzTar[2],0,0,"R");
				 $pdf->Cell(30,3,'$'.number_format($total,2,'.',','),0,0,'R');
				 ...
				 $alinea = explode("~",f_Words($vObs,150));
				 $py=25;
				 if (count($alinea) > 3){
		     ...
		     for ($k=0;$k<count($mCodDat);$k++) {
		        if($pdf->getY() > 260) {
    		     ...
    		     $pdf->SetFont('otfon1','',12);
         		 ...
         		 $pdf->Cell(10,3,str_pad($k,3,'0',STR_PAD_LEFT));
         		 ...
         		 $pdf->setXY($posx3,$posy);
	       		 ...
	       		 if ($mCodDat[$k]['pucdetxx'] == "D") { //si la cuenta detalla por DO se muestra el No del DO
