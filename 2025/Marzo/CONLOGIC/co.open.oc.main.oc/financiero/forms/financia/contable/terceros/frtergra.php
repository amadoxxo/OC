  ...
  switch ($_COOKIE['kModo']) {
    ...
    case "APLICAR":
      ...
      switch ($cAlfa) {
        ...
        case "DEDHLEXPRE":
        case "DHLEXPRE":
          //validando si tiene seleccionado regimen comun
          if ($_POST['oCliReg'] == "COMUN") {
            if (empty($_POST['oCliArr']) || empty($_POST['oCliArcr']) || $_POST['oCliArr'] == 'NO' || $_POST['oCliArcr'] == 'NO') {
              $nSwitch = 1;
              $cMsj .= "Linea ".str_pad(__LINE__,4,"0",STR_PAD_LEFT).": ";
              $cMsj .= "Si selecciona [Regimen Comun], debe marcar tanto [Agente Retenedor en Renta] como [Agente Retenedor CREE].";
            }
          }
          ...
          if ($_POST['cTerEma'] != "") {
            if (!filter_var($_POST['cTerEma'], FILTER_VALIDATE_EMAIL)) {
              $nSwitch = 1;
              $cMsj .= "Linea ".str_pad(__LINE__,4,"0",STR_PAD_LEFT).": ";
              $cMsj .= " El Correo Electronico [".$_POST['cTerEma']."], No es Valido y/o No Puede Contener mas de un Correo.\n";
            }
          }
        break;
        case "TESIACOSIP":
        ...
        case 'TEINTERLO2':
          ...
          */ 
          ...
          /**
           * Valida si el Tercero es de tipo Cliente debe digitar el plan de criterios del cliente
           */
          if (trim($_POST['cCliCPlCr']) == "" && $cChCli == "SI") {
            $nSwitch = 1;
            $cMsj .= "Linea ".str_pad(__LINE__,4,"0",STR_PAD_LEFT).": ";
            $cMsj .= " Debe Digitar el Plan Criterios del Cliente.\n";
          }
          /**
           * Valida si el Tercero es de tipo Cliente debe digitar el plan de criterios del cliente
           */
          if (trim($_POST['cCliCCrMa']) == "" && $cChCli == "SI") {
            $nSwitch = 1;
            $cMsj .= "Linea ".str_pad(__LINE__,4,"0",STR_PAD_LEFT).": ";
            $cMsj .= " Debe Digitar el Criterio Mayor del Cliente.\n";
          }
          /**
           *  Valida si el Tercero aplica para otra clasificacion diferente a Cliente debe digitar el plan de criterios del proveedor
           */
          if (trim($_POST['cCliPPlCr']) == "" && 
            ($cChEmp == "SI" || $cChCliVenCo  == "SI"|| 
            $cChProC == "SI" || $cChProE  == "SI"    || 
            $cChSoc  == "SI" || $cChEfi == "SI"      || 
            $cChOtr  == "SI" || $cChCon == "SI")) {
            $nSwitch = 1;
            $cMsj .= "Linea ".str_pad(__LINE__,4,"0",STR_PAD_LEFT).": ";
            $cMsj .= " Debe Digitar el Plan Criterios del Proveedor.\n";
          }
          /**
           *  Valida si el Tercero aplica para otra clasificacion diferente a Cliente debe digitar el Criterio Mayor del proveedor
           */
          if (trim($_POST['cCliPCrMa']) == "" && 
            ($cChEmp == "SI" || $cChCliVenCo  == "SI"|| 
            $cChProC == "SI" || $cChProE  == "SI"    || 
            $cChSoc  == "SI" || $cChEfi == "SI"      || 
            $cChOtr  == "SI" || $cChCon == "SI")) {
            $nSwitch = 1;
            $cMsj .= "Linea ".str_pad(__LINE__,4,"0",STR_PAD_LEFT).": ";
            $cMsj .= " Debe Digitar el Criterio Mayor del Proveedor.\n";
          }
        break;
        default:
          // No hace nada
  ...
  if ($nSwitch == 0) {
    ...
    switch ($_COOKIE['kModo']) {
      ...
      case "ANULAR":
        ...
        /**
         * Se Invoca el Servicio Web de Interlogistica para transmitir a SIAESA el Cliente, Proveedor o Tercero
         */
        if ($cAlfa == "INTERLO2" || $cAlfa == "TEINTERLO2" || $cAlfa == "DEINTERLO2") {
          $ObjApiSiesa   = new cIntegracionApiSiesa();

          $vDatos['cliidxxx'] = trim(strtoupper($_POST['cTerId']));
          $mRetorna = $ObjApiSiesa->fnTransmitirTerceros($vDatos);
