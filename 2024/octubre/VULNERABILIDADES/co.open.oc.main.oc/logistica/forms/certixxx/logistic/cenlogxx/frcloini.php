<?php
  namespace openComex;
/**
 * Tracking Centro Logístico.
 * --- Descripcion: permite realizar consultas rapidas Centro Logístico que se Encuentra en la Base de Datos.
...
/* Busco en la sys00039 que Tiene Permiso el Usuario*/
$qUsrMen  = "SELECT * ";
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xOrvSap,xOfvSap,xCLoSap) {
        var cPathUrl = "frclonue.php?cOrvSap="+xOrvSap+"&cOfvSap="+xOfvSap+"&cCLoSap="+xCLoSap;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Centro Logistico;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frclonue.php?cOrvSap="+mMatriz[0]+"&cOfvSap="+mMatriz[1]+"&cCLoSap="+mMatriz[2];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Centro Logistico;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de la Centro Logistico ["+mMatriz[2]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
                  document.forms['frestado']['cOrvSap'].value=mMatriz[0];
                  document.forms['frestado']['cOfvSap'].value=mMatriz[1];
                  document.forms['frestado']['cCLoSap'].value=mMatriz[2];
                  document.forms['frestado']['cEstado'].value=mMatriz[3];
                  document.cookie="kModo="+xModo+";path="+"/";
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frclogra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cOrvSap" value = "">
      <input type = "hidden" name = "cOfvSap" value = "">
      <input type = "hidden" name = "cCLoSap" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frcloini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mCenLog = array();
        $qCenLog  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qCenLog .= "$cAlfa.lpar0003.orvsapxx,";
        $qCenLog .= "$cAlfa.lpar0003.ofvsapxx,";
        $qCenLog .= "$cAlfa.lpar0003.closapxx,";
        $qCenLog .= "$cAlfa.lpar0003.clodesxx,";
        $qCenLog .= "$cAlfa.lpar0003.regusrxx,"; // Código Usuario
        $qCenLog .= "$cAlfa.lpar0003.regfcrex,"; // Fecha de creación
        $qCenLog .= "$cAlfa.lpar0003.reghcrex,"; // Hora de creación
        $qCenLog .= "$cAlfa.lpar0003.regfmodx,"; // Fecha de modificación
        $qCenLog .= "$cAlfa.lpar0003.reghmodx,"; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qCenLog .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qCenLog .= "$cAlfa.lpar0003.regestxx ";
        $qCenLog .= "FROM $cAlfa.lpar0003 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qCenLog .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0003.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qCenLog .= "WHERE ";
          $qCenLog .= "$cAlfa.lpar0003.closapxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qCenLog .= "$cAlfa.lpar0003.clodesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qCenLog .= "$cAlfa.lpar0003.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qCenLog .= "$cAlfa.lpar0003.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qCenLog .= "$cAlfa.lpar0003.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qCenLog .= "$cAlfa.lpar0003.regusrxx IN ($cNombreSearch) OR ";
          }
          $qCenLog .= "$cAlfa.lpar0003.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        $qCenLog .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xCenLog = mysql_query($qCenLog, $xConexion01, true, $cIdCountRow);
        //echo $qCenLog." ~ ".mysql_num_rows($xCenLog);
        ...
        while ($xRDC = mysql_fetch_array($xCenLog)) {
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>Centro Log&iacute;stico (<?php echo $nRNR?>)</legend>
                <center>
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="13%">
                        <a href = "javascript:fnOrder_By('onclick','orvsapxx');" title="Ordenar">Codigo SAP</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "orvsapxx">
                        <input type = "hidden" name = "orvsapxx" value = "<?php echo $_POST['orvsapxx'] ?>" id = "orvsapxx">
                        <script language="javascript">fnOrder_By('','orvsapxx')</script>
                      </td>
                      <td class="name" width="40%">
                        <a href = "javascript:fnOrder_By('onclick','clodesxx');" title="Ordenar">Centro Log&iacute;stico</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "clodesxx">
                        <input type = "hidden" name = "clodesxx" value = "<?php echo $_POST['clodesxx'] ?>" id = "clodesxx">
                        <script language="javascript">fnOrder_By('','clodesxx')</script>
                      </td>
                      <td class="name" width="18%">
                        <a href = "javascript:fnOrder_By('onclick','regusrxx');" title="Ordenar">Usuario</a>&nbsp;
                        ...
                        <script language="javascript">fnOrder_By('','regusrxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mCenLog) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mCenLog);$i++) {
                        if ($i < count($mCenLog)) { // Para Controlar el Error
                          ...
                          <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                            <td class="letra7" width="13%">
                              <a href = javascript:fnVer('<?php echo $mCenLog[$i]['orvsapxx']?>','<?php echo $mCenLog[$i]['ofvsapxx']?>','<?php echo $mCenLog[$i]['closapxx']?>')>
                                                          <?php echo $mCenLog[$i]['closapxx']?>
                              </a>
                            <td class="letra7" width="40%"><?php echo $mCenLog[$i]['clodesxx'] ?></td>
                            <td class="letra7" width="18%"><?php echo substr($mCenLog[$i]['usrnomxx'],0,20) ?></td>
                            <td class="letra7" width="07%"><?php echo $mCenLog[$i]['regfcrex'] ?></td>
                            <td class="letra7" width="05%"><?php echo $mCenLog[$i]['reghcrex'] ?></td>
                            <td class="letra7" width="07%"><?php echo $mCenLog[$i]['regfmodx'] ?></td>
                            <td class="letra7" width="05%"><?php echo $mCenLog[$i]['reghmodx'] ?></td>
                            <td class="letra7" width="05%"><?php echo $mCenLog[$i]['regestxx'] ?></td>
                            <td Class="letra7" width="02%" align="right">
                              <input type="checkbox" name="oCheck" value = "<?php echo count($mCenLog) ?>"
                              id="<?php echo $mCenLog[$i]['orvsapxx'].'~'.
                                              $mCenLog[$i]['ofvsapxx'].'~'.
                                              $mCenLog[$i]['closapxx'] ?>"
                              onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mCenLog) ?>'">
                            </td>
                          </tr>
                          <?php $y++;
                        }
