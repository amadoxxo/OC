<?php
  namespace openComex;
/**
 * Tracking Tipo de Depósito.
 * --- Este programa permite realizar consultas rapidas Tipo de Depósito que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xTdeId) {
        var cPathUrl = "frtidnue.php?cTdeId="+xTdeId;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Tipo de Deposito;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frtidnue.php?cTdeId="+mMatriz[0];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Tipo de Deposito;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de el Tipo de Deposito ["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
                  document.forms['frestado']['cTdeId'].value=mMatriz[0];
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frtidgra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cTdeId" value = "">
    ...
    <form name = "frnav" action="frtidini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mTipDep = array();
        $qTipDep  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qTipDep .= "$cAlfa.lpar0007.tdeidxxx, ";
        $qTipDep .= "$cAlfa.lpar0007.tdedesxx, ";
        $qTipDep .= "$cAlfa.lpar0007.regusrxx, "; // Código Usuario
        $qTipDep .= "$cAlfa.lpar0007.regfcrex, "; // Fecha de creación
        $qTipDep .= "$cAlfa.lpar0007.reghcrex, "; // Hora de creación
        $qTipDep .= "$cAlfa.lpar0007.regfmodx, "; // Fecha de modificación
        $qTipDep .= "$cAlfa.lpar0007.reghmodx, "; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qTipDep .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qTipDep .= "$cAlfa.lpar0007.regestxx ";
        $qTipDep .= "FROM $cAlfa.lpar0007 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qTipDep .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0007.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qTipDep .= "WHERE ";
          $qTipDep .= "$cAlfa.lpar0007.tdeidxxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qTipDep .= "$cAlfa.lpar0007.tdedesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qTipDep .= "$cAlfa.lpar0007.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qTipDep .= "$cAlfa.lpar0007.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qTipDep .= "$cAlfa.lpar0007.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qTipDep .= "$cAlfa.lpar0007.regusrxx IN ($cNombreSearch) OR ";
          }
          $qTipDep .= "$cAlfa.lpar0007.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        $qTipDep .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xTipDep = mysql_query($qTipDep, $xConexion01, true, $cIdCountRow);
        //echo $qTipDep." ~ ".mysql_num_rows($xTipDep);
        ...
        while ($xRDC = mysql_fetch_array($xTipDep)) {
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>Tipo de Dep&oacute;sito (<?php echo $nRNR?>)</legend>
                <center>
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="13%">
                        <a href = "javascript:fnOrder_By('onclick','tdeidxxx');" title="Ordenar">ID</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "tdeidxxx">
                        <input type = "hidden" name = "tdeidxxx" value = "<?php echo $_POST['tdeidxxx'] ?>" id = "tdeidxxx">
                        <script language="javascript">fnOrder_By('','tdeidxxx')</script>
                      </td>
                      <td class="name" width="40%">
                        <a href = "javascript:fnOrder_By('onclick','tdedesxx');" title="Ordenar">Tipo de Dep&oacute;sito</a>&nbsp;
                        ...
                        <script language="javascript">fnOrder_By('','tdedesxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mTipDep) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mTipDep);$i++) {
                        if ($i < count($mTipDep)) { // Para Controlar el Error
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mTipDep[$i]['tdeidxxx']?>')>
                                                        <?php echo $mTipDep[$i]['tdeidxxx']?>
                            </a>
                          <td class="letra7" width="40%"><?php echo $mTipDep[$i]['tdedesxx'] ?></td>
                          <td class="letra7" width="18%"><?php echo substr($mTipDep[$i]['usrnomxx'],0,20) ?></td>
                          <td class="letra7" width="07%"><?php echo $mTipDep[$i]['regfcrex'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mTipDep[$i]['reghcrex'] ?></td>
                          <td class="letra7" width="07%"><?php echo $mTipDep[$i]['regfmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mTipDep[$i]['reghmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mTipDep[$i]['regestxx'] ?></td>
                          <td Class="letra7" width="02%" align="right">
                            <input type="checkbox" name="oCheck" value = "<?php echo count($mTipDep) ?>"
                            id="<?php echo $mTipDep[$i]['tdeidxxx'].'~'.
                                            $mTipDep[$i]['regestxx'] ?>"
                            onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mTipDep) ?>'">
                          </td>
                        </tr>
                        <?php $y++;
                        }
