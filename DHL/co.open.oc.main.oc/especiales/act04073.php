<?php
  ...
  define("_FILELOG_","act04073.log"); // Archivo de LOG
    ...
    function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
      ...
      if ($nSwitch == 0) {
        ...
        if ($mdb){
          ...
          $qInsert .= ") VALUES (";
          $qInsert .= "\"4\", \"324\", \"\", \"1\", \"dhlexpre/texfactx/frtfvnue.php\", \"Texto Factura de Venta DHL\", \"0324\", \"\", \"\", \"btn_action-log_bg5.gif\", \"btn_action-log-disabled_bg5.gif\", \"\", \"DHLEXPRE\", \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe crea opcion de menu Texto Factura de Venta DHL en el modulo de Proyectos Especiales".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
          } else {
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError Al Crear la parametrica Texto Factura de Venta DHL";
          ...
          $qInsert .= ") VALUES (";
          $qInsert .= "\"4\", \"324\", \"1\", \"frtfvnue.php\", \"NUEVO\", \"00001\", \"Creacion Texto Factura de Venta DHL\", \"btn_action-log_bg5.gif\", \"btn_action-log-disabled_bg5.gif\", \"\", \"\", NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Crea submenu interno en la opcion de menu Texto Factura de Venta DHL: ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
          } else {
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError al Crear submenu interno en la opcion de menu Texto Factura de Venta DHL".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
          }
          ...
          $qInsert .= ") VALUES (";
          $qInsert .= "\"ADMIN\", \"4\", \"324\", \"1\", NOW(), NOW(), NOW(), \"ACTIVO\")";
          $xInsert  = mysql_query($qInsert,_CONEXION_);
          if ($xInsert) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe da acceso a la opcion de menu Texto Factura de Venta DHL en el modulo de Proyectos Especiales".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
          } else {
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError al dar acceso a la parametrica de Texto Factura de Venta DHL";
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
          }
          ...
          while ($xRU = mysql_fetch_array($xUsuarios)) {
            ...
            $qInsert .= "usridxxx, modidxxx, proidxxx, menidxxx, regfecxx, regmodxx, reghorxx, regestxx";
            $qInsert .= ") VALUES (";
            $qInsert .= "\"{$xRU['USRIDXXX']}\", \"4\", \"324\", \"1\", NOW(), NOW(), NOW(), \"ACTIVO\")";
            $xInsert  = mysql_query($qInsert,_CONEXION_);
            if ($xInsert) {
              $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe creo el permiso para el Usuario Interno [{$xRU['USRIDXXX']}] Texto Factura de Venta DHL en el modulo de Proyectos Especiales en: ".$pdb;
              system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
            } else {
              $nSwitch = 1;
              $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError al crear el permiso para el Usuario Interno [{$xRU['USRIDXXX']}] Texto Factura de Venta DHL en el modulo de Proyectos Especiales en: ".$pdb;
              system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
            }
          }
