<?php
  namespace openComex;
/**
 * Tracking Objeto Facturable.
 * --- Este programa permite realizar consultas rapidas Objeto Facturable que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xObFidx) {
        var cPathUrl = "frobfnue.php?cObFidx="+xObFidx;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Objeto Facturable;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frobfnue.php?cObFidx="+mMatriz[0];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Objeto Facturable;path="+"/";
      ...
      function fnCambiaEstado(xModo) {
          ...
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de el Objeto Facturable ["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
                  document.forms['frestado']['cObFidx'].value=mMatriz[0];
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frobfgra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cObFidx" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frobfini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mObjFac = array();
        $qObjFac  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qObjFac .= "$cAlfa.lpar0004.obfidxxx, ";
        $qObjFac .= "$cAlfa.lpar0004.obfdesxx, ";
        $qObjFac .= "$cAlfa.lpar0004.regusrxx, "; // Código Usuario
        $qObjFac .= "$cAlfa.lpar0004.regfcrex, "; // Fecha de creación
        $qObjFac .= "$cAlfa.lpar0004.reghcrex, "; // Hora de creación
        $qObjFac .= "$cAlfa.lpar0004.regfmodx, "; // Fecha de modificación
        $qObjFac .= "$cAlfa.lpar0004.reghmodx, "; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qObjFac .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qObjFac .= "$cAlfa.lpar0004.regestxx ";
        $qObjFac .= "FROM $cAlfa.lpar0004 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qObjFac .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0004.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qObjFac .= "WHERE ";
          $qObjFac .= "$cAlfa.lpar0004.obfidxxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qObjFac .= "$cAlfa.lpar0004.obfdesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qObjFac .= "$cAlfa.lpar0004.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qObjFac .= "$cAlfa.lpar0004.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qObjFac .= "$cAlfa.lpar0004.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qObjFac .= "$cAlfa.lpar0004.regusrxx IN ($cNombreSearch) OR ";
          }
          $qObjFac .= "$cAlfa.lpar0004.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        $qObjFac .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xObjFac = mysql_query($qObjFac, $xConexion01, true, $cIdCountRow);
        //echo $qObjFac." ~ ".mysql_num_rows($xObjFac);
        ...
        while ($xRDC = mysql_fetch_array($xObjFac)) {
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>Objeto Facturable (<?php echo $nRNR?>)</legend>
                <center>
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="13%">
                        <a href = "javascript:fnOrder_By('onclick','obfidxxx');" title="Ordenar">ID</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "obfidxxx">
                        <input type = "hidden" name = "obfidxxx" value = "<?php echo $_POST['obfidxxx'] ?>" id = "obfidxxx">
                        <script language="javascript">fnOrder_By('','obfidxxx')</script>
                      </td>
                      <td class="name" width="40%">
                        <a href = "javascript:fnOrder_By('onclick','obfdesxx');" title="Ordenar">Objeto Facturable</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "obfdesxx">
                        <input type = "hidden" name = "obfdesxx" value = "<?php echo $_POST['obfdesxx'] ?>" id = "obfdesxx">
                        <script language="javascript">fnOrder_By('','obfdesxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mObjFac) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mObjFac);$i++) {
                        if ($i < count($mObjFac)) { // Para Controlar el Error
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mObjFac[$i]['obfidxxx']?>')>
                                                        <?php echo $mObjFac[$i]['obfidxxx']?>
                            </a>
                          <td class="letra7" width="40%"><?php echo $mObjFac[$i]['obfdesxx'] ?></td>
                          <td class="letra7" width="18%"><?php echo substr($mObjFac[$i]['usrnomxx'],0,20) ?></td>
                          <td class="letra7" width="07%"><?php echo $mObjFac[$i]['regfcrex'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mObjFac[$i]['reghcrex'] ?></td>
                          <td class="letra7" width="07%"><?php echo $mObjFac[$i]['regfmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mObjFac[$i]['reghmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mObjFac[$i]['regestxx'] ?></td>
                          <td Class="letra7" width="02%" align="right">
                            <input type="checkbox" name="oCheck" value = "<?php echo count($mObjFac) ?>"
                            id="<?php echo $mObjFac[$i]['obfidxxx'].'~'.
                                            $mObjFac[$i]['regestxx'] ?>"
                            onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mObjFac) ?>'">
                          </td>
                        </tr>
                        <?php $y++;
                        }
