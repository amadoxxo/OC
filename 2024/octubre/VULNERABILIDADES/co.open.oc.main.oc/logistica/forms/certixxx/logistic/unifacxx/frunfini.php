<?php
  namespace openComex;
/**
 * Tracking Unidad Facturable.
 * --- Este programa permite realizar consultas rapidas Unidad Facturable que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xObFidx) {
        var cPathUrl = "frunfnue.php?cUfaId="+xObFidx;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Unidad Facturable;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frunfnue.php?cUfaId="+mMatriz[0];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Unidad Facturable;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de el Unidad Facturable ["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
                  document.forms['frestado']['cUfaId'].value=mMatriz[0];
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frunfgra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cUfaId" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frunfini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mUniFac = array();
        $qUniFac  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qUniFac .= "$cAlfa.lpar0006.ufaidxxx, ";
        $qUniFac .= "$cAlfa.lpar0006.ufadesxx, ";
        $qUniFac .= "$cAlfa.lpar0006.regusrxx, "; // Código Usuario
        $qUniFac .= "$cAlfa.lpar0006.regfcrex, "; // Fecha de creación
        $qUniFac .= "$cAlfa.lpar0006.reghcrex, "; // Hora de creación
        $qUniFac .= "$cAlfa.lpar0006.regfmodx, "; // Fecha de modificación
        $qUniFac .= "$cAlfa.lpar0006.reghmodx, "; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qUniFac .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qUniFac .= "$cAlfa.lpar0006.regestxx ";
        $qUniFac .= "FROM $cAlfa.lpar0006 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qUniFac .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0006.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qUniFac .= "WHERE ";
          $qUniFac .= "$cAlfa.lpar0006.ufaidxxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qUniFac .= "$cAlfa.lpar0006.ufadesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qUniFac .= "$cAlfa.lpar0006.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qUniFac .= "$cAlfa.lpar0006.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qUniFac .= "$cAlfa.lpar0006.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qUniFac .= "$cAlfa.lpar0006.regusrxx IN ($cNombreSearch) OR ";
          }
          $qUniFac .= "$cAlfa.lpar0006.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        $qUniFac .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xUniFac = mysql_query($qUniFac, $xConexion01, true, $cIdCountRow);
        //echo $qUniFac." ~ ".mysql_num_rows($xUniFac);
        ...
        while ($xRDC = mysql_fetch_array($xUniFac)) {
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>Unidad Facturable (<?php echo $nRNR?>)</legend>
                <center>
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="13%">
                        <a href = "javascript:fnOrder_By('onclick','ufaidxxx');" title="Ordenar">ID</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "ufaidxxx">
                        <input type = "hidden" name = "ufaidxxx" value = "<?php echo $_POST['ufaidxxx'] ?>" id = "ufaidxxx">
                        <script language="javascript">fnOrder_By('','ufaidxxx')</script>
                      </td>
                      <td class="name" width="40%">
                        <a href = "javascript:fnOrder_By('onclick','ufadesxx');" title="Ordenar">Unidad Facturable</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "ufadesxx">
                        <input type = "hidden" name = "ufadesxx" value = "<?php echo $_POST['ufadesxx'] ?>" id = "ufadesxx">
                        <script language="javascript">fnOrder_By('','ufadesxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mUniFac) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mUniFac);$i++) {
                        if ($i < count($mUniFac)) { // Para Controlar el Error
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mUniFac[$i]['ufaidxxx']?>')>
                                                        <?php echo $mUniFac[$i]['ufaidxxx']?>
                            </a>
                          <td class="letra7" width="40%"><?php echo $mUniFac[$i]['ufadesxx'] ?></td>
                          <td class="letra7" width="18%"><?php echo substr($mUniFac[$i]['usrnomxx'],0,20) ?></td>
                          <td class="letra7" width="07%"><?php echo $mUniFac[$i]['regfcrex'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mUniFac[$i]['reghcrex'] ?></td>
                          <td class="letra7" width="07%"><?php echo $mUniFac[$i]['regfmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mUniFac[$i]['reghmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mUniFac[$i]['regestxx'] ?></td>
                          <td Class="letra7" width="02%" align="right">
                            <input type="checkbox" name="oCheck" value = "<?php echo count($mUniFac) ?>"
                            id="<?php echo $mUniFac[$i]['ufaidxxx'].'~'.
                                            $mUniFac[$i]['regestxx'] ?>"
                            onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mUniFac) ?>'">
                          </td>
                        </tr>
                        <?php $y++;
                        }
