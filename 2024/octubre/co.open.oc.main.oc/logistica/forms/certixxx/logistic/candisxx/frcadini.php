<?php
  namespace openComex;
/**
 * Tracking Canal distribución.
 * --- Este programa permite realizar consultas rapidas Canal distribución que se Encuentra en la Base de Datos.
...
<html>
  <head>
    ...
    <script language="javascript">
      ...
      function fnVer(xCdiSap) {
        var cPathUrl = "frcadnue.php?cCdiSap="+xCdiSap;
        document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
        document.cookie="kMenDes=Ver Canal de Distribucion;path="+"/";
        ...
        switch (document.forms['frnav']['nRecords'].value) {
          case "1":
            if (document.forms['frnav']['oCheck'].checked == true) {
              ...
              var cPathUrl = "frcadnue.php?cCdiSap="+mMatriz[0];
              document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
              document.cookie="kMenDes=Editar Canal de Distribucion;path="+"/";
        ...
        if (document.forms['frnav']['nRecords'].value!="0"){
          switch (document.forms['frnav']['nRecords'].value) {
            case "1":
              if (document.forms['frnav']['oCheck'].checked == true) {
                var mMatriz = document.forms['frnav']['oCheck'].id.split('~');
                if (confirm("Esta Seguro de Cambiar el Estado de el Canal de Distribucion ["+mMatriz[0]+"] ?")) {
                  document.cookie="kIniAnt=<?php echo substr($_SERVER['PHP_SELF'],(strrpos($_SERVER['PHP_SELF'],"/")+1),strlen($_SERVER['PHP_SELF'])) ?>;path="+"/";
                  document.forms['frestado']['cCdiSap'].value=mMatriz[0];
  ...
  <body topmargin = "0" leftmargin = "0" rightmargin = "0" bottommargin = "0" marginheight = "0" marginwidth = "0">
    <form name = "frestado" action = "frcadgra.php" method = "post" target="fmpro">
      <input type = "hidden" name = "cCdiSap" value = "">
      <input type = "hidden" name = "cEstado" value = "">
    </form>

    <form name = "frnav" action="frcadini.php" method="post" target="fmwork">
      ...
      <?php
        ...
        $mCanDis = array();
        $qCanDis  = "SELECT DISTINCT SQL_CALC_FOUND_ROWS ";
        $qCanDis .= "$cAlfa.lpar0008.cdisapxx, ";
        $qCanDis .= "$cAlfa.lpar0008.cdidesxx, ";
        $qCanDis .= "$cAlfa.lpar0008.regusrxx, "; // Código Usuario
        $qCanDis .= "$cAlfa.lpar0008.regfcrex, "; // Fecha de creación
        $qCanDis .= "$cAlfa.lpar0008.reghcrex, "; // Hora de creación
        $qCanDis .= "$cAlfa.lpar0008.regfmodx, "; // Fecha de modificación
        $qCanDis .= "$cAlfa.lpar0008.reghmodx, "; // Hora de modificación
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qCanDis .= "IF($cAlfa.SIAI0003.USRNOMXX != \"\",$cAlfa.SIAI0003.USRNOMXX,TRIM(CONCAT($cAlfa.SIAI0003.USRPAPEX,\" \",$cAlfa.SIAI0003.USRSAPEX,\" \",$cAlfa.SIAI0003.USRPNOMX,\" \",$cAlfa.SIAI0003.USRSNOMX))) AS usrnomxx, ";
        }
        $qCanDis .= "$cAlfa.lpar0008.regestxx ";
        $qCanDis .= "FROM $cAlfa.lpar0008 ";
        if (substr_count($_POST['cOrderByOrder'],"usrnomxx") > 0) {
          $qCanDis .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.lpar0008.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
        }
        if ($_POST['vSearch'] != "") {
          $qCanDis .= "WHERE ";
          $qCanDis .= "$cAlfa.lpar0008.cdisapxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qCanDis .= "$cAlfa.lpar0008.cdidesxx LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qCanDis .= "$cAlfa.lpar0008.regfcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qCanDis .= "$cAlfa.lpar0008.reghcrex LIKE \"%{$_POST['vSearch']}%\" OR ";
          $qCanDis .= "$cAlfa.lpar0008.regfmodx LIKE \"%{$_POST['vSearch']}%\" OR ";
          if ($cNombreSearch != "") {
            $qCanDis .= "$cAlfa.lpar0008.regusrxx IN ($cNombreSearch) OR ";
          }
          $qCanDis .= "$cAlfa.lpar0008.regestxx LIKE \"%{$_POST['vSearch']}%\" ";
        }
        ...
        $qCanDis .= "$cOrderBy LIMIT $vLimInf,$vLimSup ";
        $cIdCountRow = mt_rand(1000000000, 9999999999);
        $xCanDis = mysql_query($qCanDis, $xConexion01, true, $cIdCountRow);
        //echo $qCanDis." ~ ".mysql_num_rows($xCanDis);
        ...
        while ($xRDC = mysql_fetch_array($xCanDis)) {
      ...
      <center>
        <table width="95%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td>
              <fieldset>
                <legend>Canal de Distribuci&oacute;n (<?php echo $nRNR?>)</legend>
                <center>
                  ...
                  <table cellspacing="0" width="100%">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>'>
                      <td class="name" width="13%">
                        <a href = "javascript:fnOrder_By('onclick','cdisapxx');" title="Ordenar">Cod Sap</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "cdisapxx">
                        <input type = "hidden" name = "cdisapxx" value = "<?php echo $_POST['cdisapxx'] ?>" id = "cdisapxx">
                        <script language="javascript">fnOrder_By('','cdisapxx')</script>
                      </td>
                      <td class="name" width="40%">
                        <a href = "javascript:fnOrder_By('onclick','cdidesxx');" title="Ordenar">Canal de Distribuci&oacute;n</a>&nbsp;
                        <img src="<?php echo $cPlesk_Skin_Directory_Logistic ?>/spacer.png" border="0" width="11" height="9" title = "" id = "cdidesxx">
                        <input type = "hidden" name = "cdidesxx" value = "<?php echo $_POST['cdidesxx'] ?>" id = "cdidesxx">
                        <script language="javascript">fnOrder_By('','cdidesxx')</script>
                      </td>
                    ...
                    <script languaje="javascript">
                      document.forms['frnav']['nRecords'].value = "<?php echo count($mCanDis) ?>";
                    </script>
                      <?php for ($i=0;$i<count($mCanDis);$i++) {
                        if ($i < count($mCanDis)) { // Para Controlar el Error
                        ...
                        <tr bgcolor = "<?php echo $cColor ?>" onmouseover="javascript:uRowColor(this,'<?php echo $vSysStr['system_row_select_color_ini'] ?>')" onmouseout="javascript:uRowColor(this,'<?php echo $cColor ?>')">
                          <td class="letra7" width="13%">
                            <a href = javascript:fnVer('<?php echo $mCanDis[$i]['cdisapxx']?>')>
                                                        <?php echo $mCanDis[$i]['cdisapxx']?>
                            </a>
                          <td class="letra7" width="40%"><?php echo $mCanDis[$i]['cdidesxx'] ?></td>
                          <td class="letra7" width="18%"><?php echo substr($mCanDis[$i]['usrnomxx'],0,20) ?></td>
                          <td class="letra7" width="07%"><?php echo $mCanDis[$i]['regfcrex'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mCanDis[$i]['reghcrex'] ?></td>
                          <td class="letra7" width="07%"><?php echo $mCanDis[$i]['regfmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mCanDis[$i]['reghmodx'] ?></td>
                          <td class="letra7" width="05%"><?php echo $mCanDis[$i]['regestxx'] ?></td>
                          <td Class="letra7" width="02%" align="right">
                            <input type="checkbox" name="oCheck" value = "<?php echo count($mCanDis) ?>"
                            id="<?php echo $mCanDis[$i]['cdisapxx'].'~'.
                                            $mCanDis[$i]['regestxx'] ?>"
                            onclick="javascript:document.forms['frnav']['nRecords'].value='<?php echo count($mCanDis) ?>'">
                          </td>
                        </tr>
                        <?php $y++;
                        }
