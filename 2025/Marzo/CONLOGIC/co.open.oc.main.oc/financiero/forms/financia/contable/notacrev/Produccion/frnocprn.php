    ...
    if (strlen($mPrn[$nn]) > 0) {
      ...
      $qCocDat .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.fcoc$cAno.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
  		...
  		if ($nFilCoc > 0) {
			...
			$cFacUsd = "NO"; //Por defecto siempre se imprime en pesos
			if ($cAlfa == "KARGORUX" || $cAlfa == "TEKARGORUX" || $cAlfa == "DEKARGORUX") {
				...
				// Se consulta la factura de referencia para identificar si se debe pintar en COP o USD
				$qCocDat2  = "SELECT ";
				$qCocDat2 .= "$cAlfa.fcoc$cAnoB.comobs2x ";
				...
				$qCocDat2 .= "$cAlfa.fcoc$cAnoB.comcsc2x = \"$cComCsc2B\" LIMIT 0,1";
				$xCocDat2  = f_MySql("SELECT", "", $qCocDat2, $xConexion01, "");
				$vComObs2  = array();

				if (mysql_num_rows($xCocDat2) > 0) {
					$vCocDat2 = mysql_fetch_array($xCocDat2);
					$vComObs2 = explode("~", $vCocDat2['comobs2x']);
				}

				$cFacUsd = ($vComObs2[16] == "USD") ? "SI" : "NO";
			}
  		//////////////////////////////////////////////////////////////////////////
    		...
    		// Cargo la Matriz con los ROWS del Cursor
  			...
  			while ($xRCD = mysql_fetch_array($xCodDat)) {
  			  $vCtoCon = array(); //Inicializando vector con informacion del concepto y la cuenta
					...
					// Para el cliente Kargoru se debe mostrar la descripcion personalizada de los conceptos de cobro
					if (($cAlfa == "KARGORUX" || $cAlfa == "TEKARGORUX" || $cAlfa == "DEKARGORUX") && $xRCD['comctocx'] == "IP") {
						$mCtoDesx = f_Explode_Array($xRCD['comobsxx'],"|","~");
						$vCtoDesx = explode('[', $mCtoDesx[0][2]);
						$xRCD['ctodesxx'] = $vCtoDesx[0];
					} else {
          ...
          //while ($mCodDat[$k] = mysql_fetch_array($xCodDat)) 	{ //CURSOR PARA RECORRER LA GRILLA //
              ...
              ##Fin Impresión de Logo de ADIMPEX en la parte superior derecha##
                ...
                case "TETRLXXXXX"://TRLXXXXX
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logobma1.jpg',11,6,35,10);
                break;
								case "TEADIMPEXX": // ADIMPEXX
								case "DEADIMPEXX": // ADIMPEXX
								case "ADIMPEXX": // ADIMPEXX
	              ...
	              case "DEGRUMALCO"://GRUMALCO
                	$pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logomalco.jpg',12,6,37,19);
								...
								case "DEANDINOSX": //ANDINOSX
									$pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logoAndinos2.jpeg', 20, 7,16, 18);
								break;
                ...
                case "TEMANATIAL":
                case "DEMANATIAL":
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logomanantial.jpg',12,9,36,10);
                break;
                case "DSVSASXX":
                case "TEDSVSASXX":
                case "DEDSVSASXX":
                ...
                break;
  					...
  					$pdf->Cell(18,3,$mCodDat[$k]['sccidxxx'],0,0,'L');
						...
						$cComVlr = ($cFacUsd == "SI") ? 'comvlrme' : 'comvlrxx';
						$nComVlr = ($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : $mCodDat[$k]['comvlrnf'];
						if ($mCodDat[$k]['commovxx'] == 'C') {
							...
							$SumCredito += ($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : "0";
						} else {
							//$pdf->setXY($posxcr,$posy);  //Credito //Debito
							$pdf->Cell(20,3,((strpos(($nComVlr+0),'.') > 0) ? number_format(($nComVlr+0),2,',','.') : number_format(($nComVlr+0),0,',','.')),0,0,'R');
							$pdf->Cell(20,3,'');
							$total += ($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : $mCodDat[$k]['comvlrnf'];
							$SumDebito += ($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : "0";
						}
  					///////////////// PINTO EL DETALLE /////////////////////////////
  				...
  				$pdf->SetFont('verdanab','',7);
  				$pdf->setXY($posx1+(($cFacUsd == "SI") ? 158 : 160),$posy+2);
					$pdf->Cell(20,3,(($cFacUsd == "SI") ? 'USD  ' : '$').((strpos(($SumDebito+0),'.') > 0) ? number_format(($SumDebito+0),2,',','.') : number_format(($SumDebito+0),0,',','.')),0,0,'R');
  				$pdf->Cell(20,3,(($cFacUsd == "SI") ? '' : '$').((strpos(($SumCredito+0),'.') > 0) ? number_format(($SumCredito+0),2,',','.') : number_format(($SumCredito+0),0,',','.')),0,0,'R');
  				...
  				if($j < 20) {// CUANDO SON POCOS REGISTROS  " pregunto se la grilla es menor que 20 para pintar "
  					...
  					$pdf->Cell(40,3,'TOTAL : ',0,0,'R');
						$pdf->Cell(32,3,(($cFacUsd == "SI") ? 'USD  ' : '$').((strpos(($total+0),'.') > 0) ? number_format(($total+0),2,',','.') : number_format(($total+0),0,',','.')),0,0,'R');
  		  ...
  		  if ($nFilCoc > 0 && $nFilCod > 0) {
                ...
                case "DETRLXXXXX"://TRLXXXXX
                case "TETRLXXXXX"://TRLXXXXX
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logobma1.jpg',11,$py+1,35,10);
                break;
								case "TEADIMPEXX": // ADIMPEXX
								case "DEADIMPEXX": // ADIMPEXX
								case "ADIMPEXX": // ADIMPEXX
                ...
                case "GRUMALCO"://GRUMALCO
	              ...
	              case "DEGRUMALCO"://GRUMALCO
                	$pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logomalco.jpg',12,$py+1,37,19);
								...
								case "TEANDINOSX": //ANDINOSX
								case "DEANDINOSX": //ANDINOSX
									$pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logoAndinos2.jpeg', 20, $py + 7, 16, 18);
								break;
								case "ALADUANA": //ALADUANA
								case "TEALADUANA": //ALADUANA
								case "DEALADUANA": //ALADUANA
                ...
                case "TEMANATIAL":
                case "DEMANATIAL":
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logomanantial.jpg',12,$py+4,36,10);
                break;
                case "DSVSASXX":
                case "TEDSVSASXX":
                case "DEDSVSASXX":
                ...
                break;
								...
								case "DEHAYDEARX":
								case "TEHAYDEARX":
									$pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logohaydear.jpeg',14,$py+2,35,15);
								break;
								case "CONNECTA":
								case "DECONNECTA":
								case "TECONNECTA":
              ...
              $pdf->SetFont('verdana','',7);
  					...
  					$pdf->Cell(18,3,$mCodDat[$k]['sccidxxx'],0,0,'L');
						...
						if ($mCodDat[$k]['commovxx'] == 'C') {
							//$pdf->setXY($posxdb,$posy); //Debito //Credito
							$pdf->Cell(20,3,'');
							$pdf->Cell(20,3,number_format((($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : $mCodDat[$k]['comvlrnf']),0,',','.'),0,0,'R');
							$SumCredito1 += ($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : "0";
						} else {
							//$pdf->setXY($posxcr,$posy);  //Credito //Debito
							$pdf->Cell(20,3,number_format((($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : $mCodDat[$k]['comvlrnf']),0,',','.'),0,0,'R');
							$pdf->Cell(20,3,'');
							$total1 += ($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : $mCodDat[$k]['comvlrnf'];
							$SumDebito1 += ($mCodDat[$k][$cComVlr] > 0) ? $mCodDat[$k][$cComVlr] : "0";
						}
						///////////////// PINTO EL DETALLE /////////////////////////////
  				...
  				// Para q muestre la suma del debito y credito
  				$pdf->SetFont('verdanab','',7);
  				$pdf->setXY($posx1+(($cFacUsd == "SI") ? 158 : 160),$posy-5);
  				$pdf->Cell(20,3,(($cFacUsd == "SI") ? 'USD  ' : '$').number_format($SumDebito1,0,',','.'),0,0,'R');
  				$pdf->Cell(20,3,(($cFacUsd == "SI") ? '' : '$').number_format($SumCredito1,0,',','.'),0,0,'R');
  				//Fin mostras sumatorias
  				...
  				if($P < 20) {// CUANDO SON POCOS REGISTROS  " pregunto si la grilla es menor que 20 para pintar "
  					...
  					$pdf->Cell(32,3,(($cFacUsd == "SI") ? 'USD  ' : '$').number_format($total1,0,',','.'),0,0,'R');
  					$py1+=1;
