  ...
  if ($nTotAne == 1 && isset($_POST['nId'])) {
    ...
    if ($zip->open($tmpFile, ZipArchive::CREATE) === TRUE) {
      ...
      while($xRAN = mysql_fetch_array($xAnexo)) {
        ...
        $cNameIni = $xRAN['anexname'] .".". $xRAN['anexexte'];
        $cFileNam = $cNameIni;
        $nExisten = 0;

        // Iterar sobre los archivos del ZIP para contar duplicados si existen
        for ($i = 0; $i < $zip->numFiles; $i++) {
          $cFileZip = $zip->getNameIndex($i);
          if (basename($cFileZip) === $cNameIni || basename($cFileZip) === $cFileNam) {
            $nExisten++;
            $cFileNam = $xRAN['anexname'] . '_' . $nExisten . '.' . $xRAN['anexexte'];
          }
        }

        // Agregar el archivo al .zip con un nombre único
        $zip->addFromString($cFileNam, $file);
      }
