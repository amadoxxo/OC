      ...
      $qCodDat .= "LEFT JOIN $cAlfa.SIAI0150 ON $cAlfa.fcod$cAno.teridxxx = $cAlfa.SIAI0150.CLIIDXXX ";
          ...
          // # DE PAGINAS //
              ...
              ##Fin Impresión de Logo de ADIMPEX en la parte superior derecha##
                ...
                case "DEALADUANA": //ALADUANA
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logoaladuana.jpg',16,8,30,15);
								...
								case "DEANDINOSX": //ANDINOSX
									$pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logoAndinos2.jpeg', 20, $py + 7, 20, 18);
								break;
                ...
                case "DEADUAMARX": //ADUAMARX
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logoaduamar.jpg',21,6.5,19);
								...
								case "TEPROSERCO":
								case "DEPROSERCO":
									$pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logoproserco.png', 12, 6, 37, 19);
								break;
                case "MANATIAL":
                case "TEMANATIAL":
                case "DEMANATIAL":
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logomanantial.jpg', 12, 9, 35, 10);
                break;
                case "DSVSASXX":
                case "TEDSVSASXX":
                case "DEDSVSASXX":
                ...
                break;
                  ...
                  $pdf->MultiCell(100, 3, utf8_decode($cNombreAduana . "\n NIT. " . $cNitAduana), 0, 'C'); // TITULO DEL FORMULARIO //
					      ...
					      $pdf->setXY(50,22);
								if(($cAlfa == "ACODEXXX" || $cAlfa == "DEACODEXXX" || $cAlfa == "TEACODEXXX") && $vCocDat['comtcoxx'] == "AUTOMATICO"){
									...
									if ($vCocDat['comtcoxx'] == "MANUAL") {
										$pdf->setXY(175,22);
										$pdf->Cell(30,3,utf8_decode($vCocDat["comtcbxx"]." ".$vCocDat['comcscxx']),0,0,'C');
									}
								} else {
				      ...
				      $pdf->setXY(10,28);
       				...
       				$pdf->Cell(100,3,substr($vCocDat['GIRNOMAX'],0,65));
       				// Nit del Proveedor //
       				$pdf->setXY(140,32);
       				$pdf->Cell(40,3,"NIT: {$vCocDat['terid3xx']}-".f_Digito_Verificacion($vCocDat['terid3xx']));
  					...
  					$pdf->Cell(18,3,$mCodDat[$k]['sccidxxx'],0,0,'L');
						...
						if($mCodDat[$k]['ctodesxg'] != 'CONCEPTO SIN DESCRIPCION'){
						  $pdf->cell(38,3,substr($mCodDat[$k]['ctodesxg'],0,22),0,0,'L');
						}else{
						  $pdf->cell(38,3,substr($mCodDat[$k]['pucdesxx'],0,22),0,0,'L');
						}
  				...
  				if($j < 20) {// CUANDO SON POCOS REGISTROS  " pregunto se la grilla es menor que 20 para pintar "
  					...
  					$pdf->setXY(138,$py-20);
  					$pdf->Cell(40,3,'TOTAL : ',0,0,'R');
  					$pdf->Cell(30,3,'$'.number_format(((strpos(($totalcr+0),'.') > 0) ? number_format(($totalcr+0),2,',','.') : number_format(($totalcr+0),0,',','.')),0,',','.'),0,0,'R');
  					///////////////////////////////////////////////////////
  					$posy+=8;
  		  ...
  		  if ($nFilCoc > 0 && $nFilCod > 0) {
				      ...
				      $pdf->Line(175,$py+14,175,$py+21);
  						...
  						//$pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/emision_cero.jpg',10,$py,200,30);
  					 	...
  					 	switch($cAlfa){
  							...
  							case "TEADUACARX":
  							case "DEADUACARX":
  							  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/aduacarga1.png',13,$py+4,35,15);
  							break;
  							case "ALPOPULX"://ALPOPULAR
  							case "TEALPOPULP"://ALPOPULAR PRUEBAS
  							case "DEALPOPULX"://ALPOPULAR PRUEBAS
                ...
                case "TEMANATIAL":
                case "DEMANATIAL":
                    $pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logomanantial.jpg', 12, $py + 4, 35, 10);
                break;
                case "DSVSASXX":
                case "TEDSVSASXX":
                case "DEDSVSASXX":
