    ...
    function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
      ...
      if ($nSwitch == 0) {
        ...
        if ($mdb){
          ...
          ###################################
          //Creación de Tabla Autorización Excluir Servicios
          ###################################
          $qCreate  = "CREATE TABLE lpar0160 ( ";
          $qCreate .= "aesidxxx INT(10) NOT NULL AUTO_INCREMENT COMMENT \"Id de la Autorizacion Excluir Servicios\",";
          ...
          $qCreate .= "PRIMARY KEY (aesidxxx),";
          $qCreate .= "INDEX (cliidxxx),";
          $qCreate .= "FOREIGN KEY (cliidxxx) REFERENCES lpar0150(cliidxxx),";
          $qCreate .= "FOREIGN KEY (sersapxx) REFERENCES lpar0011(sersapxx),";
          $qCreate .= "FOREIGN KEY (sersapxx, subidxxx) REFERENCES lpar0012(sersapxx, subidxxx)";
          $qCreate .= ") COMMENT=\"Autorización Excluir Servicios\";";
          $xCreate  = mysql_query($qCreate,_CONEXION_);   
          if ($xCreate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Creo la Tabla lpar0160 en: ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
          } else {
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qCreate." ~ ".mysql_error(_CONEXION_)."\tError Al Crear la Tabla lpar0160 en: ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
          }

          ###################################
          // Creación de Tabla Autorización Modificar Campos Pedido
          ###################################
          $qCreate  = "CREATE TABLE lpar0161 (";
          $qCreate .= "amcidxxx INT(10) NOT NULL AUTO_INCREMENT COMMENT \"Id de la Autorizacion Modificar Campos Pedido\",";
          $qCreate .= "cliidxxx varchar(20) NOT NULL COMMENT \"Id del Cliente\",";
          ...
          $qCreate .= "regestxx varchar(10) NOT NULL COMMENT \"Estado del Registro\",";
          $qCreate .= "regstamp timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT \"Modificado\",";
          $qCreate .= "PRIMARY KEY (amcidxxx),";
          $qCreate .= "INDEX (cliidxxx),";
          $qCreate .= "FOREIGN KEY (cliidxxx) REFERENCES lpar0150(cliidxxx),";
          $qCreate .= "FOREIGN KEY (sersapxx) REFERENCES lpar0011(sersapxx),";
          $qCreate .= "FOREIGN KEY (sersapxx, subidxxx) REFERENCES lpar0012(sersapxx, subidxxx)";
          $qCreate .= ") COMMENT=\"Autorización Modificar Campos Pedido\";";
          $xCreate = mysql_query($qCreate,_CONEXION_);
          if ($xCreate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Creo la Tabla lpar0161 en: ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
          } else {
            $nSwitch = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qCreate." ~ ".mysql_error(_CONEXION_)."\tError Al Crear la Tabla lpar0161 en: ".$pdb;
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
          }
