  ...
  for ($nn=0;$nn<count($mPrn);$nn++) {
    if (strlen($mPrn[$nn]) > 0) {
      ...
      $qCocDat .= "LEFT JOIN $cAlfa.SIAI0003 ON $cAlfa.fcoc$cAno.regusrxx = $cAlfa.SIAI0003.USRIDXXX ";
  		...
  		///////////////////////////////////////////////////
			/* CARGO MATRIZ DE FORMAS DE PAGO PARA LOS RECIBOS DE CAJA*/
			$zMatrizFprc = explode("|",$vCocDat['comfpxxx']);
			$zMatrizAce   = array();
			$z = 0;
			//f_Mensaje(__FILE__,__LINE__,count($zMatrizFprc));
			for ($i=0;$i<count($zMatrizFprc);$i++) {
				if (!empty($zMatrizFprc[$i])) {
					$zMatrizFpr = explode("~",$zMatrizFprc[$i]);
					$zMatrizAce[$z]['formapxx'] = $zMatrizFpr[0];
					$zMatrizAce[$z]['refpcxxx'] = $zMatrizFpr[1];
					$zMatrizAce[$z]['bancodes'] = $zMatrizFpr[2];
					$zMatrizAce[$z]['valorxxx'] = $zMatrizFpr[5];
					$z++;
				}
			}
      ///////////////////////////////////////////////////////////////////////////
		  ...
		  if ($nFilCoc > 0 || $nFilCod > 0) {
    		...
    		for ($k=0;$k<count($mCodDat);$k++) {
              ...
              $pdf->Line(10,$py,208,$py);
                  ...
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logobma1.jpg',12,$py+5,35,10);
							  break;
								case "TEADIMPEXX": // ADIMPEX
								case "DEADIMPEXX": // ADIMPEX
								case "ADIMPEXX": // ADIMPEX
                ...
                case "GRUMALCO": //GRUMALCO
                  ...
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logomalco.jpg',14,$py + 1,33,19);
	  						break;
                ...
                case "DEALADUANA": //ALADUANA
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logoaladuana.jpg',16,$py+4,30,15);
								...
								case "DEANDINOSX": //ANDINOSX
									$pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logoAndinos2.jpeg', 19, $py + 1, 18, 19);
								break;
								...
								case "DEADUAMARX": //ADUAMARX
									$pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logoaduamar.jpg',20,$py+1.5,19);
								break;
								...
								case "DEKARGORUX": //KARGORUX
									$pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logokargoru.jpg', 12.5, $py + 3, 35, 15);
								break;
								case "ALOGISAS": //LOGISTICA
								case "TEALOGISAS": //LOGISTICA
								case "DEALOGISAS": //LOGISTICA
									$pdf->Image($_SERVER['DOCUMENT_ROOT'] . $cPlesk_Skin_Directory . '/logologisticasas.jpg', 11, $py + 3, 38);
								break;
								case "PROSERCO":
								case "TEPROSERCO":
								case "DEPROSERCO":
                ...
                case "TEMANATIAL":
                case "DEMANATIAL":
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logomanantial.jpg',12,$py+6,35);
                break;
                case "DSVSASXX":
                case "TEDSVSASXX":
                case "DEDSVSASXX":
                ...
                case "DEHAYDEARX":
                case "TEHAYDEARX":
                  $pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/logohaydear.jpeg',14,$py+3,35,16);
                break;
                case "CONNECTA":
                case "DECONNECTA":
                case "TECONNECTA":
                ...
                break;
								default://Logo open
									$pdf->Image($_SERVER['DOCUMENT_ROOT'].$cPlesk_Skin_Directory.'/opentecnologia.JPG',11,$py+3,35,15);
								break;
	  					}
	  					##Impresion de Logos Agencias de Aduanas Financiero Contable 2011-05-27 Yulieth Campos##
                  ...
                  $pdf->MultiCell(100, 3, utf8_decode($cNombreAduana . "\n NIT. " . $cNitAduana), 0, 'C'); // TITULO DEL FORMULARIO //
								...
								$py+=6;
								//$pdf->setXY(50,$py);
								//$pdf->Cell(100,3,utf8_decode("NIT. ".$cNitAduana),0,0,'C'); // NIT //
								$py+=5;
								$pdf->setXY(50,$py);
								$pdf->Cell(100,3,utf8_decode($vCocDat['comdesxx']),0,0,'C'); // TITULO DEL FORMULARIO //
					      $py+=4;
					      $pdf->setXY(50,$py);
								#consulta para saber si es de tipo manual o automatico e imprimir segun corresponda
								#$vCocDat['comcsc3x'] --> consecutivo 3
	            ...
	            // Cliente //
	       			...
	       			//Numero comprobante
	  				  ...
	  				  $pdf->SetFont('verdanab','',7);
	  					$pdf->setXY($posx1,$py);
	  					$pdf->Cell(30,3,'FORMA PAGO',0,0,"C");
	  					$pdf->Cell(30,3,'REF PAG/CHE',0,0,"C");
	  					$pdf->Cell(100,3,'BANCO DESCRIPCION',0,0,"C");
	  					$pdf->Cell(38,3,'VALOR',0,0,"R");

	  					$totrec=0;
	  					$py+=10;
	            for ($z=0;$z<count($zMatrizAce);$z++) {
	      				$pdf->SetFont('verdana','',7);
	      				$pdf->setXY($posx1,$py);
	      				$pdf->Cell(30,3,$zMatrizAce[$z]['formapxx'],0,0,"L");
	      				$pdf->Cell(30,3,$zMatrizAce[$z]['refpcxxx'],0,0,"C");
	      				$totrec+=$zMatrizAce[$z]['valorxxx'];

	      				if($zMatrizAce[$z]['formapxx'] == "CHEQUE" || $zMatrizAce[$z]['formapxx'] == "TRANSFERENCIA") {
	      					$qDatBan = "SELECT * FROM $cAlfa.fpar0124 WHERE banidxxx = \"{$zMatrizAce[$z]['bancodes']}\" LIMIT 0,1 ";
	                $xDatBan = f_MySql("SELECT","",$qDatBan,$xConexion01,"");
	                $zRDB = mysql_fetch_array($xDatBan);
					       	$pdf->Cell(100,3,$zMatrizAce[$z]['bancodes']."-".trim($zRDB['bandesxx']),0,0,"C");
	      				}else{
	      					$pdf->Cell(100,3,$zMatrizAce[$z]['bancodes'],0,0,"C");
	      				}
	      				$pdf->Cell(38,3,number_format($zMatrizAce[$z]['valorxxx'],0,',','.'),0,0,'R');
	      				$py+=3;
	            }
	            ...
	            $py+=3;
	    				$pdf->SetFont('verdanab','',7);
	    				$pdf->setXY(138,$py);
	    				$pdf->Cell(40,3,'TOTAL RECIBIDO: ',0,0,'R');
	    				$pdf->Cell(30,3,'$'.number_format($totrec,0,',','.'),0,0,'R');
  					}
  					$py+=5;
       			// DATOS DE DETALLE //
  			...
  			$pdf->SetFont('verdana','',7);
  			$pdf->SetXY(10,$nP+3);
  			$pdf->Cell(200,3,trim($vCocDat['comobsxx'],"~"));
