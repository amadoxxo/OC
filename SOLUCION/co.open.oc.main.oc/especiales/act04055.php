<?php
  ...
  define("_FILELOG_","act04055.log"); // Archivo de LOG
  ...
  for ($i=0;$i<count($aconn);$i++){
    $vdb  = $aconn[$i]['DB'];
    $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tActualiza las descripciones de los medios de pago en la tabla fpar0155: ".$vdb;
    ...
    function fnEjecutarQueries($pInicio,$pFinal,$pdb) {
      ...
      if ($nSwitch == 0) {
        ...
        if ($mdb){
          ...
          $cDescripcion = utf8_decode("DESEMBOLSO CRÉDITO (CCD)");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"17\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          } else {
            $nSwicht = 1;
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_error(_CONEXION_)."\t".$qUpdate." ~ ".mysql_error(_CONEXION_)."\tError Al Actualizar Datos";
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
          }

          $cDescripcion = utf8_decode("DESEMBOLSO (CCD) DÉBITO");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"18\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $cDescripcion = utf8_decode("DESEMBOLSO DÉBITO PLUS (CCD+)");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"33\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $cDescripcion = utf8_decode("DESEMBOLSO CRÉDITO (CCD)");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"35\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $cDescripcion = utf8_decode("DESEMBOLSO DÉBITO (CCD)");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"36\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $cDescripcion = utf8_decode("CRÉDITO INTERCAMBIO CORPORATIVO (CTX)");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"39\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $cDescripcion = utf8_decode("DESEMBOLSO CRÉDITO PLUS (CCD+)");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"41\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $cDescripcion = utf8_decode("DESEMBOLSO DÉBITO PLUS (CCD+)");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"43\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
          ...
          $cDescripcion = utf8_decode("TELEX ESTÁNDAR BANCARIO");
          $qUpdate = "UPDATE fpar0155 SET mpadesxx = \"$cDescripcion\" WHERE fpar0155.mpaidxxx = \"51\"";
          $xUpdate  = mysql_query($qUpdate,_CONEXION_);
          if ($xUpdate) {
            $cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qUpdate." ~ ".mysql_affected_rows(_CONEXION_);
            system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n"; // echo $cMsj."\n";
