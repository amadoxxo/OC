...
define("_FILELOG_","act04150.log"); // Archivo de LOG
		...
		if ($nSwitch == 0) {
			...
			if ($mdb){
				...
				if ($xInsert) {
					...
					$nSwitch = 1;
					$cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError Al Crear Nuevo Concepto de Cobro en la tabla fpar0129: ".$pdb;
				...
				$xInsert  = mysql_query($qInsert,_CONEXION_);
				if ($xInsert) {
					$cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\tSe Creo la Asociacion de la Condicion Especial Contra el Concepto y Forma de Cobro en la tabla fpar0145: ".$pdb;
					system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;34m".$cMsj."\33[00m\n";
				} else {
					$nSwitch = 1;
					$cMsj = date("Y-m-d H:m:s")."\t".str_pad(__LINE__,4,"0",STR_PAD_LEFT)."\t".number_format(microtime(true)-_MICROTIME_,2)."\t".$qInsert." ~ ".mysql_error(_CONEXION_)."\tError al Crear la Asociacion de la Condicion Especial Contra el Concepto y Forma de Cobro en la tabla fpar0145: ".$pdb;
					system("/bin/echo -e '".$cMsj."' >> "._DIRLOG_._FILELOG_); echo "\33[01;01;91m".$cMsj."\33[00m\n";
				}
