														...
														<?php while ($mCiuDes = mysql_fetch_array($xCiuDes)) {
															if (mysql_num_rows($xCiuDes) > 1) { ?>
																<tr>
																	<td width = "050" class= "name">
																		<a href = "javascript:window.opener.document.forms['frgrm']['cCiuIdDc'].value  ='<?php echo $mCiuDes['ciuidxxx']?>';
																													window.opener.document.forms['frgrm']['cCiuDesDc'].value ='<?php echo $mCiuDes['ciudesxx']?>';
															...
															<?php	} else { ?>
																<script languaje="javascript">
																	window.opener.document.forms['frgrm']['cCiuIdDc'].value  = '<?php echo $mCiuDes['ciuidxxx'] ?>';																
																	window.opener.document.forms['frgrm']['cCiuDesDc'].value = '<?php echo $mCiuDes['ciudesxx'] ?>';
																	close();
																</script>																	
															<?php }
	  								...
	  								case "VALID":	  									
	  									...
	  									if ($xCiuDes && mysql_num_rows($xCiuDes) > 0) {
	  										while ($mCiuDes = mysql_fetch_array($xCiuDes)) { ?>
													<script languaje = "javascript">
														parent.fmwork.document.forms['frgrm']['cCiuIdDc'].value  = '<?php echo $mCiuDes['ciuidxxx'] ?>';													
														parent.fmwork.document.forms['frgrm']['cCiuDesDc'].value = '<?php echo $mCiuDes['ciudesxx'] ?>';
														close();
													</script>
	      	      				<?php break;
	  										}
