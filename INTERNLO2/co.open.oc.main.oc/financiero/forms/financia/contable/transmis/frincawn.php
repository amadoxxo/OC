<?php
  /**
   * Generacion Archivo Integracion CagoWise.
   * --- Descripcion: Permite la Generacion Archivo Integracion CagoWise.
  ...
  $qSysProbg .= "pbatinxx = \"TRANSMICARGOWISE\" ";
  ...
  while ($xRB = mysql_fetch_array($xSysProbg)) {
    ...
    if (count($mArcProBg[$nInd_mArcProBg]['pbaexcxx']) == 0) {
    ...
    <script language = "javascript">
      ...
      function f_Links(xLink,xSwitch,xSecuencia,xGrid,xType) {
    		...
    		switch (xLink) {
    		  case "cComCod":
    			case "cComDes":
    				if (document.forms['frgrm']['cInterfaz'].value != ""){
    				  if (xLink == "cComCod" || xLink == "cComDes") {
                var cInterfaz = document.forms['frgrm']['cInterfaz'].value.toUpperCase();
      ...
      function fnEnvia(){
				...
				var indcom = document.frgrm.cInterfaz.selectedIndex;
            ...
            alert('Fecha Final no puede ser mayor a la Fecha de Hoy,verifique');
  					...
  					if (fini < inii){
  						...
  						var zWinPro  = 'width=500,scrollbars=1,height=250,left='+zNx+',top='+zNy;
							var cEjProBg = document.forms['frgrm']['cEjProBg'].value; //Proceso en Background
							var cRuta    = 'frincawg.php?dDesde='    +dDesde+'&dHasta='+dHasta+
                                          '&gInterfaz='+document.forms['frgrm']['cInterfaz'].options[indcom].value+
    ...
    <center>
      <table border ="0" cellpadding="0" cellspacing="0" width="560">
        <tr>
          <td>
            <fieldset>
            <legend>Generaci&oacute;n Archivo Integraci&oacute;n CargoWise</legend>
              <form name = "frgrm" action = "frsiigo.php" method = "post" target="fmwork">
                <table border = "0" cellpadding = "0" cellspacing = "0" style = "width:560">
                  ...
                  <table border = "0" cellpadding = "0" cellspacing = "0" style = "width:560">
                    ...
                    <tr>
                      ...
                      <td Class = "name" colspan = "12">Interfaz<br>
                        <input type = 'hidden' name = 'cComId'> 
                        <select class="letrase" size="1" name="cInterfaz" style = "width:240"
                          onchange="javascript: document.forms['frgrm']['cComId'].value  = '';
                                                document.forms['frgrm']['cComCod'].value = '';
                                                document.forms['frgrm']['cComDes'].value = '';">
                          <!-- <option value = "RECIBOS_CAJA">RECIBOS DE CAJA</option> -->
                          <option value = "CAUSACIONES_TERCEROS">CAUSACIONES TERCEROS / CARTAS BANCARIAS</option>
                          <option value = "FACTURAS">FACTURAS</option>
                          <option value = "NOTAS_CREDITO">NOTAS CR&Eacute;DITO</option>
                        </select>
                      </td>
                    </tr>
                    <tr>
                ...
                <label>
                  <table border="0" cellspacing="1" cellpadding="0" width="560">
                    <tr bgcolor = '<?php echo $vSysStr['system_row_title_color_ini'] ?>' style="height:20px">
                      ...
                      <td align="center" style="padding:2px"><strong>Filtros</strong></td>
                    ...
                    <tr bgcolor = "<?php echo $cColor ?>">
                      ...
                      <td style="padding:2px">
                        ...
                        <?php if ($mArcProBg[$i]['gInterfaz'] != "") { ?>
                          <strong>Interfaz: </strong><?php echo $mArcProBg[$i]['gInterfaz'] ?>
                        <?php } ?>
                      ...
                      <td style="padding:2px">
                        <?php if (is_array($mArcProBg[$i]['pbaexcxx'])) { 
                          for ($nA=0; $nA<count($mArcProBg[$i]['pbaexcxx']); $nA++) {
                            ?>
                            <a href = "javascript:fnDescargar('<?php echo $mArcProBg[$i]['pbaexcxx'][$nA]; ?>')">
                              <?php echo $mArcProBg[$i]['pbaexcxx'][$nA] ?>
                            </a>
                            <?php
                          }
                        } elseif ($mArcProBg[$i]['pbaexcxx'] != "") { ?>
                          <a href = "javascript:fnDescargar('<?php echo $mArcProBg[$i]['pbaexcxx']; ?>')">
                            <?php echo $mArcProBg[$i]['pbaexcxx'] ?>
                          </a>
                        <?php } ?>
